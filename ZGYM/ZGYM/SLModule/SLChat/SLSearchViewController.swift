...
class SLSearchViewController: SLBaseTableViewController {
    ...
    // MARK: - Action
    @objc func cancelClick(sender:SLButton) {
            ...
            c?.getMessage(1, last: nil, succ: { (msgs) in
                if msgs?.count ?? 0 > 0 && msgs?.first is TIMMessage{
                    ...
                    if SLChatHelper.sharedInstance.isServiceMessage(msg: msg) {
                        model.subTitle = SLChatHelper.sharedInstance.msg2IMCustomModel(msg: msg).content ?? ""
                    }
...
//        cell.changeColorWhenTouched = true
    ...
    // MARK: - LazyLoad
    lazy var searchBar: SLSearchView = {
        let view = SLSearchView(frame: CGRect.zero)
        view.lbTitle.font = UIFont.systemFont(ofSize: 14)
        view.tfInput.font = UIFont.systemFont(ofSize: 14)
        view.tfInput.textColor = UIColor.sl_hexToAdecimalColor(hex: "#575A60")
    ...
    override func image(forEmptyDataSet scrollView: UIScrollView) -> UIImage? {
        return  NightNight.theme == nil ? UIImage.init(named: "sl_empty_search_night") : UIImage(named: "sl_empty_search")
    }

    override func title(forEmptyDataSet scrollView: UIScrollView) -> NSAttributedString? {
        let text = "没有找到相关内容"
        let attributes = [NSAttributedString.Key.font:UIFont.systemFont(ofSize: CGFloat(18)),
                          NSAttributedString.Key.foregroundColor: UIColor.sl_hexToAdecimalColor(hex: "#898F9A")]
