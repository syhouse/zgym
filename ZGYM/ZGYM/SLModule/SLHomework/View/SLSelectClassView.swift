...
class SLSelectClassView: UIControl {
    override init(frame: CGRect) {
        super.init(frame: frame)
        self.addSubview(selectTipslabel)
        self.addSubview(selectLabel)
        self.addSubview(imageView)
        mixedBackgroundColor = MixedColor(normal: UIColor.white, night: UIColor.sl_hexToAdecimalColor(hex: "#20232F"))
        ...
        for index in 0..<maxcount{
            ...
            label.mixedBackgroundColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#F3F5F9"), night: kNight282C3B)
    ...
    lazy var selectTipslabel: SLLabel = {
        let label = SLLabel()
        label.font = kTextMainBodyFont
    ...
    lazy var imageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_class_arrow"))
        return imageView
    }()
}
