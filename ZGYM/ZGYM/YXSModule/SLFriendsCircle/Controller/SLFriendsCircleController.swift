...
class SLFriendsCircleController: YXSBaseTableViewController {
    // MARK: - 属性
    ...
    ///朋友圈详情
    func loadDetialData(){
        YXSEducationClassCircleTimeAxisDetailRequest.init(classCircleId: classCircleId ?? 0).request({ (model:SLFriendCircleModel) in
    ...
    func loadDelectCommentData(_ section: Int = 0,commentModel: SLFriendsCommentModel){
        ...
        UIUtil.yxs_changeCommentFriendCirclePrise(SLFriendCircleModel.classCircleId ?? 0, commentId: commentModel.id ?? 0,positon: positon) { [weak self](model) in
    ...
    func loadCommentData(_ section: Int = 0,classCircleId: Int,content: String,commentModel: SLFriendsCommentModel?){
        ...
        UIUtil.yxs_changeCommentFriendCirclePrise(SLFriendCircleModel.classCircleId ?? 0,commentModel: commentModel,content: content,positon: positon) { [weak self](model) in
            guard let strongSelf = self else { return }
    ...
    @objc func rightClick(){
        if YXSPersonDataModel.sharePerson.personRole == .PARENT{
            ...
            YXSEducationClassOptionalGradeListRequest().requestCollection({ (classs:[SLClassModel]) in
                //                self.classes = classs
    ...
    func recallEvent(_ section: Int){
        ...
        let SLFriendCircleModel = dataSource[section]
        UIUtil.yxs_loadRecallData(.friendCicle, id: SLFriendCircleModel.classCircleId ?? 0,positon: positon) {
    ...
    func changePrise(_ section: Int){
        ...
        let SLFriendCircleModel = dataSource[section]
        UIUtil.yxs_changeFriendCirclePrise(SLFriendCircleModel,positon: positon) {
...
// MARK: - MediaHelper
extension SLFriendsCircleController: YXSSelectMediaHelperDelegate{
...
extension SLFriendsCircleController{
    ...
    @objc func yxs_updateListForComment(_ notification:Notification){
        ...
        if let isAddComment = userInfo?[kNotificationIsDelectKey] as? Bool,let commentModel = userInfo?[kNotificationModelKey] as? SLFriendsCommentModel,let classCircleId = userInfo?[kNotificationIdKey] as? Int{
            for (_,SLFriendCircleModel) in dataSource.enumerated(){
