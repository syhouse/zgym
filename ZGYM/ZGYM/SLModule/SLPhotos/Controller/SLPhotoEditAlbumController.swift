//
//  SLPhotoEditAlbumController.swift
//  ZGYM
//
...
class SLPhotoEditAlbumController: SLEditAlbumBaseController {
    var updateAlbumSucess: ((_ albumModel: SLPhotoAlbumsModel)->())?
    let albumModel: SLPhotoAlbumsModel
    // MARK: -leftCycle
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "编辑相册"
        let rightButton = SLButton(frame: CGRect(x: 0, y: 0, width: 58, height: 25))
    ...
    override func loadUploadFinishData(){
        ...
        SLEducationAlbumUpdateAlbumNameOrCoverRequest.init(id: albumModel.id ?? 0, albumName: nameField.text, coverUrl: coverUrl).request({ (result) in
    ...
    @objc func deleteClick(){
        MBProgressHUD.sl_showLoading()
        SLEducationAlbumDeleteRequest.init(id: albumModel.id ?? 0).request({ (result) in
            MBProgressHUD.sl_showMessage(message: "删除成功")
            if let viewControllers = self.navigationController?.viewControllers{
                for vc in viewControllers{
                    if vc is SLPhotoClassPhotoAlbumListController{
                        let newVC = vc as! SLPhotoClassPhotoAlbumListController
                        newVC.removeAlbum(albumModel: self.albumModel)
