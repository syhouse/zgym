...
//  ZGYM
//
//  Created by hnsl_mac on 2019/12/23.
//  Copyright © 2019 hnsl_mac. All rights reserved.
//

import UIKit

enum SLShareType: Int{
    case img
    case link
}

class SLShareModel: NSObject {
    var type: SLShareType
    var way: SLShareWayType
    var image: UIImage?
    var title: String?
    var descriptionText: String?
    var link: String?
    private init(type: SLShareType,way: SLShareWayType) {
        self.type = type
        self.way = way
        super.init()
    }
    ...
    convenience init(way: SLShareWayType = .QQSession,image: UIImage){
    ...
    /// - Parameter link: 链接地址
    convenience init(way: SLShareWayType = .QQSession,image: UIImage = UIImage.init(named: "sl_logo")!,title: String,descriptionText: String,link: String){
...
class HMRequestShareModel: NSObject {
    ...
    convenience init(classStarId: Int,childrenId: Int?,dateType: DateType,startTime:String? = nil,endTime:String? = nil){
        ...
        let dayStr = NSUtil.sl_timeDayText(dateType: dateType)
