...
class SLSetTopAlertView: UIView {

        var complete: ((_ sender: SLButton)->())?
    var topButtonTitle: String
        @discardableResult static func showIn(target: UIView,topButtonTitle: String = "置顶", complete:((_ sender: SLButton)->())?) -> SLSetTopAlertView{
            let view = SLSetTopAlertView(topButtonTitle: topButtonTitle)
        ...
        func layout() {
            ...
            // MARK: - Action
            @objc func setTopClick(sender: SLButton) {
                self.complete?(sender)
                self.panelView.removeFromSuperview()
            }
            
            @objc func cancelClick(_ sender: SLButton = SLButton()) {
                self.complete?(sender)
                self.panelView.removeFromSuperview()
            }
