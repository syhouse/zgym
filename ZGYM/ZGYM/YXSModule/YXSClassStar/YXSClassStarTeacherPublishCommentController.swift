//
//  YXSClassStarTeacherPublishCommentController.swift
//  ZGYM
//
...
class YXSClassStarTeacherPublishCommentController: YXSBaseViewController {
    ...
    var dataSource: [YXSClassStarChildrenModel]!
    init(model: YXSClassStartClassModel? , isPublish: Bool = false,classId: Int? = nil) {
        self.model = model
    ...
    var selectComments: [YXSClassStarChildrenModel]!{
        get{
            var selectComments = [YXSClassStarChildrenModel]()
    ...
    @objc func yxs_loadClassTopData() {
        YXSEducationClassStarTeacherClassTopRequest.init(classId: classId, dateType: model?.dateType ?? DateType.W).requestCollection({ (classs:[YXSClassStartClassModel]) in
    ...
    func yxs_loadClassChildrensData(){
        ...
        YXSEducationClassStarTeacherClassChildrenTopRequest.init(classId: model.classId ?? 0, sortType: Int(publishView.selectModel.paramsKey) ?? 0, dateType: model.dateType).requestCollection({ (list:[YXSClassStarChildrenModel]) in
            self.dataSource = list
            let allClassModel = YXSClassStarChildrenModel.init(JSON: ["" : ""])!
...
//            self.group.leave()
    ...
    func yxs_loadEvaluationListData(){
        ...
        YXSEducationFEvaluationListListRequest.init(classId: model.classId ?? 0,stage: model.stageType).requestCollection({ (list: [YXSClassStarCommentItemModel]) in
    ...
    func yxs_dealComment(_ model: YXSClassStarChildrenModel?){
        ...
        var selectComments: [YXSClassStarChildrenModel] = self.selectComments
        //点击评论
        if let model = model{
            selectComments.append(model)
        }
    ...
    lazy var publishView: YXSClassStarTeacherPublishView = {
        let publishView = YXSClassStarTeacherPublishView()
        return publishView
    }()
...
extension YXSClassStarTeacherPublishCommentController: YXSRouterEventProtocol{
    func yxs_user_routerEventWithName(eventName: String, info: [String : Any]?) {
        ...
        case kYXSClassStarTeacherPublishViewDoCommentEvent:
            yxs_dealComment(info?[kEventKey] as? YXSClassStarChildrenModel)
        default:
            break
        }
    }
