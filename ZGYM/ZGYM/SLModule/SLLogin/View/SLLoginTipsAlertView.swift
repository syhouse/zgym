...
class SLLoginTipsAlertView: UIView {
    
    /// 展示
    /// - Parameter complete: 点击所有按钮回调
    static func showAlert(complete:(( _ type: LoginAlertTipsType) ->())?) -> SLLoginTipsAlertView{
        let view = SLLoginTipsAlertView()
    ...
    private let service: String = "《优学生软件用户服务协议》"
    private let privacy: String = "《优学生用户隐私保护政策》"
    ...
    @objc func buttonClick(button: SLButton){
...
//        view.textColor = kTextLightBlackColor
    ...
    lazy var serviceButton : SLButton = {
    ...
    lazy var privacyButton : SLButton = {
        let button = SLButton.init()
        button.setTitleColor(kBlueColor, for: .normal)
...
//        button.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA")
