...
/// 指引弹出Window
class YXSGuideViewWindow: UIControl{
    private var items:[YXSGuideModel]
    private var sucess: (()->())?
    ...
    ///   - onView: window的父视图
    ///   - sucess: 指引完成回调
    static func showGuideViewWindow(items:[YXSGuideModel],onView: UIView = UIUtil.RootController().view,sucess:(()->())? = nil){
        let view = YXSGuideViewWindow.init(items: items, onView: onView)
        view.sucess = sucess
        view.beginAnimation()
    }
    
    private init(items:[YXSGuideModel],onView: UIView = UIUtil.RootController().view){
        self.items = items
    ...
    private func setUI(model:YXSGuideModel?){
        if let model = model{
            self.removeSubviews()
            if model.type == .view{
