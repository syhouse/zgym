...
//    var isPause: Bool = true
    ...
    var model: SLVoiceViewModel? {
        didSet {
            lbSecond.text = "\(self.model?.voiceDuration ?? 0)\""
            /// 装载音频
            if let url = URL(string: model?.voiceUlr ?? "") {
                SLSSAudioPlayer.sharedInstance.play(url: url){
                    self.stopVoiceAnimation()
                }
                SLSSAudioPlayer.sharedInstance.pauseVoice()
            }
    ...
    @objc func indicatorClick(sender: UIControl) {
        if tapPlayer{
            ...
            SLLog(">>>>sourceUrl:\(SLSSAudioPlayer.sharedInstance.sourceUrl?.absoluteString ?? "")")
            SLLog(">>>>voiceUlr:\(self.model?.voiceUlr ?? "")")
            if SLSSAudioPlayer.sharedInstance.sourceUrl?.absoluteString != self.model?.voiceUlr {
                if let url = URL(string: model?.voiceUlr ?? "") {
                    SLSSAudioPlayer.sharedInstance.play(url: url){ [weak self] in
                ...
                if SLSSAudioPlayer.sharedInstance.isPause {
                    SLSSAudioPlayer.sharedInstance.resumeVoice()
                    startVoiceAnimation()
    ...
    func startVoiceAnimation(){
        ...
        let ary = [
            UIImage(named: "sl_homework_voice0"),
            UIImage(named: "sl_homework_voice1"),
            UIImage(named: "sl_homework_voice2"),
        ]
    ...
    lazy var indicatorView: UIControl = {
        let view = UIControl()
        view.mixedBackgroundColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#7CDCDF"), night: UIColor.sl_hexToAdecimalColor(hex: "#96DADE"))
    ...
    lazy var imgIcon: UIImageView = {
        let img = UIImageView()
        img.image = UIImage(named: "sl_homework_voice2")
        return img
    }()
    
    lazy var lbSecond: SLLabel = {
        let lb = SLLabel()
    ...
    lazy var closeButton: SLButton = {
        let closeButton = SLButton()
        closeButton.setImage(UIImage.init(named: "sl_publish_close"), for: .normal)
...
class SLVoiceViewModel: NSObject {
    var voiceUlr: String?
