...
let kYMClassStartTeacherDetailStudentEmptyHeaderViewRemindEvent = "SLClassStartTeacherDetailStudentEmptyHeaderViewRemindEvent"
class SLClassStartTeacherDetailStudentEmptyHeaderView: UIView {
    override init(frame: CGRect) {
        super.init(frame: frame)
        backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9")
        addSubview(bgView)
    ...
    @objc func goRemind(){
        next?.sl_routerEventWithName(eventName: kYMClassStartTeacherDetailStudentEmptyHeaderViewRemindEvent)
    }
    ...
    func setHeaderModel(_ model: SLChildrenModel, dateType: DateType){
        titleLabel.text = "\(NSUtil.sl_getDateText(dateType: dateType))暂无点评"
        ...
        UIUtil.sl_setLabelAttributed(commentLabel, text: ["全国已有", "200万", "名学生正在使用此功能，班级之星是学生德育培养的重要辅助功能，赶快去提醒老师使用吧！"], colors: [UIColor.sl_hexToAdecimalColor(hex: "#BABDC2"),kBlueColor, UIColor.sl_hexToAdecimalColor(hex: "#BABDC2")])
        
        let height = UIUtil.sl_getTextHeigh(textStr: "全国已有" + "200万" + "名学生正在使用此功能，班级之星是学生德育培养的重要辅助功能，赶快去提醒老师使用吧！", font: UIFont.systemFont(ofSize: 14), width: SCREEN_WIDTH - 94)
    ...
    lazy var titleLabel: SLLabel = {
        let label = SLLabel()
        label.textColor = UIColor.sl_hexToAdecimalColor(hex: "#6A6C71")
        label.font = UIFont.systemFont(ofSize: 18)
        return label
    }()
    
    lazy var commentLabel: SLLabel = {
        let label = SLLabel()
        label.textColor = UIColor.sl_hexToAdecimalColor(hex: "#6A6C71")
    ...
    lazy var goRemindButton: SLButton = {
