...
class YXSGlobalJumpManager: NSObject {
    ...
    @objc func yxs_jumpBy(model: IMCustomMessageModel, fromViewControllter: UIViewController? = nil) {
        ...
        case 100:
            ...
            let visibleVC = fromViewControllter ?? getVisibleVC(inTabBarController: tabBar, index: 0)
            let vc = YXSPunchCardSingleStudentBaseListController.init(clockInId: model.callbackRequestParameter?.clockInId ?? 0, clockInCommitId: model.callbackRequestParameter?.clockInCommitId ?? 0, isMyPublish: false, classId: model.classId ?? 0, topHistoryModel: nil)
            visibleVC?.navigationController?.pushViewController(vc)
            hasJumpEnd = true
            break
