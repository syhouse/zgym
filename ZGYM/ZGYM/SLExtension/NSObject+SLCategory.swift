...
extension NSObject{
    ...
    var sl_user: SLEducationUserModel {
        get{
            SLPersonDataModel.sharePerson.userModel
        }
    }
    
    var sl_childrenClassList: [[String: Int]]{
        get{
            var childrenClassList = [[String: Int]]()
            if let children = sl_user.children{
    ...
    func sl_startTime(_ curruntDate: Date = Date(), curruntDay: String? = nil) -> String{
    ...
    func sl_dealList(list:[SLHomeListModel],childId: Int?, isAgenda: Bool) -> [SLHomeListModel]{
        if SLPersonDataModel.sharePerson.personRole == .TEACHER{
            return list
        }
        var newList = [SLHomeListModel]()
        for model in list{
            if isAgenda{
                for info in self.sl_childrenClassList{
                    if model.classId == info["classId"]{
                        if model.childrenId == nil{
                            ...
                            if let newModel = newModel as? SLHomeListModel{
    ...
    func sl_dealList(list:[SLSolitaireModel],childId: Int?, isAgenda: Bool) -> [SLSolitaireModel]{
        if isAgenda{
            for model in list{
                if model.state == nil{
                    model.state = 10
    ...
    func sl_dealList(list:[SLPunchCardModel],childId: Int?, isAgenda: Bool) -> [SLPunchCardModel]{
        if SLPersonDataModel.sharePerson.personRole == .TEACHER || isAgenda{
            return list
        }
        var newList = [SLPunchCardModel]()
        for model in list{
            model.childrenId = childId
            newList.append(model)
        }
    ...
    func sl_weakText(periodList: [Int]?) -> String{
        var weakText = "周期:"
    ...
    //当天日历天是否是今天
    func sl_isToDay(_ curruntDate: Date = Date(), compareDate: Date? = Date(),curruntDay dayCount: String? = nil) -> CalendarDateCompare{
        var curruntDay = "1"
        let dateFormatter = DateFormatter()
        var curruntYear = ""
        var curruntMonth = ""
    ...
    // MARK: -二维码
    //MARK: -传进去字符串,生成二维码图片
    public func sl_setupQRCodeImage(_ text: String, image: UIImage?) -> UIImage {
        //创建滤镜
...
// MARK: - HM String
extension NSObject{
    func sl_classFromText(className: String?, teacherName: String?, createTime: String?) -> String{
        var text = ""
        
        let maxCount = 13
        var maxTeacherNameCount = (teacherName ?? "").count > 4 ? 4 : (teacherName ?? "").count
        let maxClassNameCount = maxCount - maxTeacherNameCount
        if let className = className{
            let classText = className.count > maxClassNameCount ? "\(className.mySubString(to: maxClassNameCount))" : className
            maxTeacherNameCount = maxCount - classText.count
            text += classText + (className.count > maxClassNameCount ? "..." : "")
        }
        text += "|"
        if let teacherName = teacherName{
            text += teacherName.count > maxTeacherNameCount ? "\(teacherName.mySubString(to: maxTeacherNameCount))..." : teacherName
        }
        text += "|\(Date.sl_Time(dateStr: createTime ?? ""))"
        return text
    }
}
    ...
    @objc func greetingTextFieldChanged(obj:Notification, length: Int = 10) {
        if let textField = obj.object as? UITextField{
            guard let _:UITextRange = textField.markedTextRange else {
                ...
                //替换后的字符串(已过滤非中文字符) subjectField.text!.sl_pregReplace(pattern: pattern, with: "")
    ...
    @objc func greetingTextFieldChanged(obj:Notification, charslength: Int = 10) {
        if let textField = obj.object as? UITextField{
            guard let _:UITextRange = textField.markedTextRange else {
                ...
                var str = textField.text!
                //限制最大输入长度
                if str.sl_numberOfChars() > charslength {
                    str = str.sl_lengtToChars(length: charslength)
                }
