...
class SLClassAddController: SLBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let rightButton = sl_setRightButton(image: "scan")
    ...
    @objc func lookClassClick(){
        ...
        SLEducationGradeQueryRequest.init(num: classField.text!).request({ (model: SLClassQueryResultModel) in
            MBProgressHUD.hide(for: self.view, animated: true)
            if model.id == nil{
                self.view.makeToast("未查找到班级")
            }else{
                let vc = SLClassQueryListController.init(model)
    ...
    // MARK: - getter&setter
    lazy var questionView: SLCustomImageControl = {
        let questionView = SLCustomImageControl.init(imageSize: CGSize.init(width: 15, height: 15), position: .left, padding: 7.5)
        questionView.font = UIFont.systemFont(ofSize: 15)
        questionView.textColor = UIColor.sl_hexToAdecimalColor(hex: "#898F9A")
        questionView.title = "如何获取班级号？"
        questionView.locailImage = "sl_class_quest"
        questionView.addTarget(self, action: #selector(questionClick), for: .touchUpInside)
        return questionView
    }()
    
    lazy var classField: SLQSTextField = {
        let classField = UIUtil.sl_getTextField(UIEdgeInsets.init(top: 0, left: 15, bottom: 0, right: 0), placeholder: "请输入老师给您的班级号", placeholderColor: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), mixedTextColor:MixedColor(normal: kTextMainBodyColor, night: UIColor.white))
    ...
    lazy var lookClassButton: SLButton = {
