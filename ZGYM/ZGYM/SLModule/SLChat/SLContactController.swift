//
//  SLContactController.swift
//  ZGYM
//
//  Created by sl_mac on 2019/12/11.
...
/// 私聊联系人
class SLContactController: SLBaseTableViewController{
    ...
    /// 所有组的数据
    var dataDict:[String:Array<SLContactModel>] = Dictionary()
    /// 组标题
    var groupList:[String] = [String]()
    /// 特殊字符组的Cells数据(不包含老师组里的特殊字符成员)
    var nonameList:[SLContactModel] = [SLContactModel]()
    /// 老师组的Cells数据
    var teacherList:[SLContactModel] = [SLContactModel]()
    /// 家长组的Cells数据
    var parentList:[SLContactModel] = [SLContactModel]()
...
//        setupRightBarButtonItem()
        ...
        /// 索引字颜色
        tableView.sectionIndexColor = kNightBCC6D4//UIColor.sl_hexToAdecimalColor(hex: "#575A60")
        tableView.register(SLContactCell.classForCoder(), forCellReuseIdentifier: "SLContactCell")
        tableView.register(SLContactSectionHeaderView.classForCoder(), forHeaderFooterViewReuseIdentifier: "SLContactSectionHeaderView")
...
//        MBProgressHUD.sl_showLoading()
        SLEducationTeacherQueryContactsRequest(classId: classesModel?[currentClassIndex].id ?? 0).requestCollection({ [weak self](list:[SLContactModel]) in
            guard let weakSelf = self else {return}
//            MBProgressHUD.sl_hideHUD()
...
//            MBProgressHUD.sl_hideHUD()
            ...
            /// 去重
            model = model.sl_filterDuplicates({$0.id})
            model = weakSelf.filter(classList: model)
            weakSelf.classesModel = model
...
//        MBProgressHUD.sl_showLoading()
        SLEducationParentQueryContactsRequest(classId: classesModel?[currentClassIndex].id ?? 0).requestCollection({ [weak self](list:[SLContactModel]) in
            guard let weakSelf = self else {return}
//            MBProgressHUD.sl_hideHUD()
            weakSelf.contactsModel = list
            weakSelf.processData(list: list)
    ...
    var contactsModel: [SLContactModel]? {
        didSet {
//            tableView.reloadData()
    ...
    @objc func rightBarButtonClick(sender:UIButton) {
        let vc = SLConversationListController()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:SLContactCell = tableView.dequeueReusableCell(withIdentifier: "SLContactCell") as! SLContactCell
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let headerView: SLContactSectionHeaderView = tableView.dequeueReusableHeaderFooterView(withIdentifier: "SLContactSectionHeaderView") as! SLContactSectionHeaderView
        let model = SLContactSectionHeaderViewModel()
...
//    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let cell: SLContactCell = tableView.cellForRow(at: indexPath) as! SLContactCell
        self.sl_pushChatVC(imId: cell.model?.imId ?? "")
    }
    ...
    @objc func filterTeacher(list:[SLContactModel] ,completionHandler:((_ teacherList:[SLContactModel], _ parentList:[SLContactModel])->())) {
        
        teacherList.removeAll()
        var teacherListTmp:[SLContactModel] = [SLContactModel]()
        var parentListTmp:[SLContactModel] = [SLContactModel]()
        for sub in list {
    ...
    /// 排序
    @objc func processData(list:[SLContactModel]) {
        ...
        filterTeacher(list: list) { [weak self](teacherList, parentList) in
            ...
            for sub in parentList {
                ...
                /// 存
                var list:[SLContactModel]? = dataDict[firstString]
                if (list == nil) {
                    list = [SLContactModel]()
                    list?.append(sub)
    ...
    // MARK: - LazyLoad
    lazy var gradeView: SLContactGradeView = {
        let flow = UICollectionViewFlowLayout()
        flow.scrollDirection = .horizontal
        let gradeView = SLContactGradeView(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH, height: 55), collectionViewLayout: flow)
        return gradeView
    }()
...
class SLBadgeButton: UIControl {
    ...
    override init(frame: CGRect) {
        ...
        button = SLButton(frame: CGRect.zero)
