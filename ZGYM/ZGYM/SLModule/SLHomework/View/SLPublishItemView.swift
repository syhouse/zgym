...
class SLPublishViewButtonView: UIView{
    ...
    var buttons: [[String: Any]]{
        get{
            var lists = [[kImageKey: "sl_publish_image", kActionKey: PublishViewButtonEvent.image],[kImageKey: "sl_publish_vedio", kActionKey: PublishViewButtonEvent.vedio]]
            if SLPersonDataModel.sharePerson.personRole == .TEACHER && !isFriend{
                lists.append([kImageKey: "sl_publish_link", kActionKey: PublishViewButtonEvent.link])
            }
            if !isFriend{
                lists.insert([kImageKey: "sl_publish_audio", kActionKey: PublishViewButtonEvent.audio], at: 0)
    ...
    var buttonClickBlock: ((_ event:PublishViewButtonEvent) -> ())?
    
    func changeButtonStates(_ sourceTypes:[PublishSource]){
        for index in 0..<buttons.count{
            let button = viewWithTag(index + publishViewButtonOrginTag) as? SLButton
...
class SLPublishLinkView: UIView {
    ...
    func createUI() {
        ...
        self.mixedBackgroundColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#F3F5F9"), night: kNight282C3B)
    ...
    // MARK: - Action
    @objc func linkClick(sender: SLButton) {
        clickCompletion?()
    }
    @objc func close() {
        closeCompletion?()
    ...
    // MARK: - LazyLoad
    lazy var btnLink: SLButton = {
        let btn = SLButton()
        btn.setTitle("https://www.baidu.com", for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        btn.titleLabel?.lineBreakMode = .byTruncatingTail
        btn.contentHorizontalAlignment = .left
        btn.setMixedTitleColor(MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#575A60"), night: kNightBCC6D4), forState: .normal)
        btn.addTarget(self, action: #selector(linkClick(sender:)), for: .touchUpInside)
        return btn
    }()
    
    lazy var imgIcon: SLButton = {
        let img = SLButton()
        img.setImage(UIImage(named: "sl_publish_delect_gray"), for: .normal)
