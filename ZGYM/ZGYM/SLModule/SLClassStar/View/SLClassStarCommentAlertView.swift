//
//  SLClassStarCommentAlertView.swift
//  ZGYM
//
...
class ClassStarCommentTotalModel: NSObject{
    ...
    var childrenIds: [Int]!
    var stage: StageType
    init(titles: [String],dataSource: [[SLClassStarCommentItemModel]],classId: Int,stage: StageType) {
        self.titles = titles
...
class SLClassStarCommentAlertView: UIView {
    @discardableResult static func showClassStarComment(model: ClassStarCommentTotalModel,sucess:((_ item:SLClassStarCommentItemModel)->())?) -> SLClassStarCommentAlertView{
        let view = SLClassStarCommentAlertView(model: model,sucess:sucess)
        view.beginAnimation()
        return view
    }
    let model: ClassStarCommentTotalModel
    let sucess:((_ item:SLClassStarCommentItemModel)->())?
    init(model: ClassStarCommentTotalModel,sucess:((_ item: SLClassStarCommentItemModel)->())?){
        self.model = model
        self.sucess = sucess
    ...
    func pushWithItemModel(item: SLClassStarCommentItemModel,defultIndex: Int){
        ...
        //剔除系统的
        var firstCleanList = [SLClassStarCommentItemModel]()
        var secendCleanList = [SLClassStarCommentItemModel]()
        ...
        if item.itemType == .Edit{
            ...
            if model.stage == .KINDERGARTEN{
                newModel.dataSource[0].insert(SLClassStarCommentItemModel.getYMClassStarCommentItemModel(.Delect, title: "删除"), at: 0)
                newModel.dataSource[0].insert(SLClassStarCommentItemModel.getYMClassStarCommentItemModel(.Add, title: "添加"), at: 0)
            }else{
                newModel.dataSource[0].insert(SLClassStarCommentItemModel.getYMClassStarCommentItemModel(.Delect, title: "删除"), at: 0)
                newModel.dataSource[0].insert(SLClassStarCommentItemModel.getYMClassStarCommentItemModel(.Add, title: "添加"), at: 0)
                newModel.dataSource[1].insert(SLClassStarCommentItemModel.getYMClassStarCommentItemModel(.Delect, title: "删除"), at: 0)
                newModel.dataSource[1].insert(SLClassStarCommentItemModel.getYMClassStarCommentItemModel(.Add, title: "添加"), at: 0)
            }
            UIUtil.curruntNav().pushViewController(SLClassStarCommentEditItemListController.init(totalModel: newModel, defultIndex: defultIndex))
            dismiss()
        }else{
            //do comment
            MBProgressHUD.showAdded(to: self.bgWindow, animated: true)
            SLEducationClassStarTeacherEvaluationChildrenRequest.init(childrenIds: model.childrenIds, classId: model.classId, evaluationId: item.id ?? 0).request({ (result) in
                ...
                MBProgressHUD.hide(for: self.bgWindow, animated: true)
                MBProgressHUD.sl_showMessage(message: msg, inView: self.bgWindow)
            }
        }
    ...
    lazy var titleLabel : UILabel = {
        ...
        view.mixedTextColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#000000"), night: kNightFFFFFF)
    ...
    lazy var categoryView :JXCategoryTitleView = {
        ...
        view.titleColor = UIColor.sl_hexToAdecimalColor(hex: "#575A60")
...
extension SLClassStarCommentAlertView: JXCategoryListContainerViewDelegate, JXCategoryViewDelegate{
    func number(ofListsInlistContainerView listContainerView: JXCategoryListContainerView!) -> Int {
        return model.titles.count
    }
    
    func listContainerView(_ listContainerView: JXCategoryListContainerView!, initListFor index: Int) -> JXCategoryListContentViewDelegate! {
        let vc = SLClassStarCommentItemController.init(dataSource: model.dataSource[index])
