...
//  ZGYM
//
//  Created by sl_mac on 2019/12/25.
...
class SLInviteViewController: SLBaseViewController {
...
//            make.height.equalTo(364*SCREEN_SCALE)
    ...
    @objc func refreshData() {
        ...
        SLEducationShareLinkRequest(model: model).request({ [weak self](json) in
            ...
            weakSelf.imgQRCode.image = weakSelf.sl_setupQRCodeImage(weakSelf.qrCodeUrl ?? "", image: nil)
    ...
    // MARK: - Action
    @objc func saveClick(sender: SLButton) {
   ...
   @objc private func saveImage(image: UIImage, didFinishSavingWithError error: NSError?, contextInfo: AnyObject) {
      ...
      MBProgressHUD.sl_showMessage(message: showMessage)
   }
    
    @objc func shareClick(sender: SLButton) {
        let model = SLShareModel(image: screenShortWithElement(view: contentView))
        SLShareTool.showCommonShare(shareModel: model)
    }
    
    @objc func copyClick(sender: SLButton) {
        let pastboard = UIPasteboard.general
        pastboard.string = gradeNum
        MBProgressHUD.sl_showMessage(message: "复制成功")
    }
    ...
    lazy var scrollView: UIScrollView = {
        ...
        scrollView.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9")
        return scrollView
    }()
    
    lazy var contentView: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#4E87FF")
        return view
    }()
    
    lazy var imgBgTop: UIImageView = {
        let img = UIImageView()
        img.image = UIImage(named: "sl_invite_bg_top")
        return img
    }()
    
    lazy var imgBgBottom: UIImageView = {
        let img = UIImageView()
        img.image = UIImage(named: "sl_invite_bg_bottom")
        return img
    }()
    ...
    lazy var lbClassNumber: SLLabel = {
        let lb = SLLabel()
        lb.text = "班级号："
    ...
    lazy var lbClassNumberContent: SLLabel = {
        let lb = SLLabel()
        lb.font = UIFont.systemFont(ofSize: 19)
        lb.textColor = UIColor.sl_hexToAdecimalColor(hex: "#5E88F7")
        return lb
    }()
    
    lazy var lbNick: SLLabel = {
        let lb = SLLabel()
        lb.text = "亲爱的家长/老师，您好！"
    ...
    lazy var lbContent: SLLabel = {
        let lb = SLLabel()
        lb.text = "    为了确保每位家长都可以及时查看老师发的消息，我们现在使用“优学生”APP作为家校沟通的工具。\n    老师发的消息不会被刷屏，及时了解孩子班级动态。"
        lb.numberOfLines = 0
        lb.textColor = UIColor.sl_hexToAdecimalColor(hex: "#898F9A")
        lb.font = UIFont.systemFont(ofSize: 15)
        return lb
    ...
    lazy var imgConnect1: UIImageView = {
        let img = UIImageView()
        img.image = UIImage(named: "sl_invite_aaa")
        return img
    }()
...
//    }()
    ...
    lazy var btnCopy: SLButton = {
        let btn = SLButton()
        btn.setTitle("复制", for: .normal)
        btn.titleEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: -10)
        btn.setTitleColor(UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        btn.setImage(UIImage(named: "sl_invite_copy"), for: .normal)
    ...
    lazy var btnSave: SLButton = {
        let btn = SLButton()
        btn.setTitle("保存到手机", for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 17)
        btn.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F4BE63")
    ...
    lazy var btnShare: SLButton = {
        let btn = SLButton()
        btn.setTitle("分享二维码", for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 17)
        btn.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#89DBFD")
        btn.clipsToBounds = true
        btn.layer.cornerRadius = 5
...
extension SLInviteViewController: SLRouterEventProtocol{
