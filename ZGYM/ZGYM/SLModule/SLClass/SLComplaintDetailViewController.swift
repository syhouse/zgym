//
//  SLComplaintDetailViewController.swift
//  ZGYM
//
...
class SLComplaintDetailViewController: SLBaseViewController {
    ...
    @objc func uploadImagesRequest(completionHandler:((_ result:String)->())?) {
        ...
        if infos.count > 0{
            ...
            SLUploadSourceHelper().uploadMedia(mediaInfos: infos, sucess: { [weak self](list) in
                guard let weakSelf = self else {return}
                SLLog(infos)
                MBProgressHUD.sl_hideHUDInView(view: weakSelf.navigationController!.view)
...
//                        bgUrl = model[urlKey] as? String ?? ""
    ...
    @objc func doneClick(sender: SLButton) {
        ...
        if self.firstView.textView.text.count < 1 {
            MBProgressHUD.sl_showMessage(message: "描述内容不能为空")
            return
        }
        ...
        uploadImagesRequest { [weak self](screenshots) in
            ...
            SLEducationComplaintSubmitRequest(respondentId: weakSelf.respondentId, respondentType: weakSelf.respondentType, type: weakSelf.type, content: weakSelf.firstView.textView.text ?? "", screenshots: screenshots).request({ [weak self](json) in
                guard let weakSelf = self else {return}
                MBProgressHUD.sl_hideHUD()
                MBProgressHUD.sl_showMessage(message: "提交成功")
                DispatchQueue.main.asyncAfter(deadline: .now()+1.0) {
                    weakSelf.navigationController?.sl_existViewController(existClass: SLFriendsCircleInfoController(userId: 0, childId: 0, type: ""), complete: { (isContain, resultVC) in
...
// MARK: -
class SLComplaintImagesView: UIView,SLSelectMediaHelperDelegate {
    /// 是添加图片点击
    private let maxcount = 3
    private var isShowMedia: Bool = false
    private var isAdd: Bool = false
    var publishModel: SLPublishModel = SLPublishModel()
    /// 是否已选择发布视频
    ...
    func showSelectMedia(_ isAdd: Bool){
        ...
        if isAdd{
            SLSelectMediaHelper.shareHelper.pushImagePickerController(mediaStyle: .onlyImage,maxCount: 3)
        }else{
            SLSelectMediaHelper.shareHelper.pushImagePickerController(assets,mediaStyle: .onlyImage,maxCount: maxcount)
        }
