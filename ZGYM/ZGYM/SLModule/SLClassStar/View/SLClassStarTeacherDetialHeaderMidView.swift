...
/// 中间排名视图
class SLClassStarTeacherDetialHeaderMidView: UIView{
    ...
    @objc func detailControlClick(){
        next?.sl_routerEventWithName(eventName: kYMClassStarTeacherDetialHeaderViewLookDetialEvent)
    }
    ...
    func setHeaderModel(_ model: SLClassStartClassModel){
        if model.hasComment{
            ...
            rightRankView.midImageView.image = UIImage.init(named: "sl_classstar_crown_gray")
            leftRankView.midImageView.image = UIImage.init(named: "sl_classstar_student_gray")
        }
    }
    ...
    lazy var bgImageView: UIView = {
        ...
        imageView.addShadow(ofColor: UIColor.sl_hexToAdecimalColor(hex: "#D5DFFB"), radius: 7.5, offset: CGSize(width: 0, height: 3), opacity: 0.5)
        return imageView
    }()
    
    lazy var bottomLabelImageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_classstar_teacher_class_single_detial_mid_bg"))
        return imageView
    }()
...
class SLRankView: UIView{
    ...
    lazy var bottomImageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_classstar_platform"))
        return imageView
    }()
    ...
    lazy var labelImageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_classstar_crown"))
        return imageView
    }()
    ...
    lazy var noDataView: SLButton = {
        let noDataView = SLButton()
        noDataView.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        noDataView.setTitleColor(UIColor.sl_hexToAdecimalColor(hex: "#898F9A"), for: .normal)
        noDataView.setTitle("暂无", for: .normal)
        noDataView.cornerRadius = 12
        noDataView.isHidden = true
        noDataView.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#E9E9E9")
        return noDataView
    }()
}


class ClassStarProgressView: SLButton {
    ...
    /// 前景颜色
    public var foregroundColor: UIColor = UIColor.sl_hexToAdecimalColor(hex: "#FFCA28")
    ...
    override func draw(_ rect: CGRect) {
        ...
        if  praiseScore == 0 && improveScore == 0{
            ...
            UIColor.sl_hexToAdecimalColor(hex: "#DAE4FC").set()
            restPath.stroke()
        }else{
