...
class YXSPasswordView: UIView, UIKeyInput {
    //输入的文本
    ...
    override func draw(_ rect: CGRect) {
        if bgView.subviews.count == 0{
            ...
            for index in 0..<maxLength{
                let view = YXSLabel()
                view.mixedTextColor = MixedColor(normal: UIColor.yxs_hexToAdecimalColor(hex: "#575A60"), night: UIColor.white)
                ...
                view.borderColor = UIColor.yxs_hexToAdecimalColor(hex: "#E6EAF3")
    ...
    lazy var bgView: UIView = {
        let bgView = UIView()
        bgView.mixedBackgroundColor = MixedColor(normal: UIColor.yxs_hexToAdecimalColor(hex: "#F2F5F9"), night: kNightBackgroundColor)
        return bgView
    }()
}
