//
//  SLClassStarTeacherDetialHeaderView.swift
//  ZGYM
//
...
let kYMClassStarTeacherDetialHeaderViewLookDetialEvent = "SLClassStarTeacherDetialHeaderViewLookDetialEvent"

class SLClassStarTeacherDetialHeaderView: UIView {
    let stage: StageType
    ...
    func setHeaderModel(_ model: SLClassStartClassModel?){
        if let model = model{
    ...
    lazy var bottomView: SLClassStarTeacherDetialHeaderMidView = {
        let bottomView = SLClassStarTeacherDetialHeaderMidView()
        return bottomView
    }()
...
class ClassStarTeacherDetialHeaderKINDERGARTENView: UIView{
    ...
    lazy var leftCommont: SLCustomImageControl = {
        ...
        let leftCommont = SLCustomImageControl.init(imageSize: CGSize.init(width: 21.5, height: 23), position: SLImagePositionType.top, padding: 7.5)
        leftCommont.textColor = UIColor.white
        leftCommont.locailImage = "sl_classstar_cicle_huaduo"
        leftCommont.textLabel.textAlignment = .center
        return leftCommont
    }()
    lazy var rightCommont: SLCustomImageControl = {
        
        let rightCommont = SLCustomImageControl.init(imageSize: CGSize.init(width: 21.5, height: 23), position: SLImagePositionType.top, padding: 7.5)
        rightCommont.textColor = UIColor.white
        rightCommont.locailImage = "sl_classstar_cicle_zongrenshu"
...
class ClassStarTeacherDetialHeaderOtherLevelView: UIView{
    ...
    func setHeaderModel(_ model: SLClassStartClassModel){
        self.model = model
        if model.hasComment{
            UIUtil.sl_setLabelAttributed(averageLabel, text: ["班级平均分：", "\(model.averageScore ?? "")"], fonts: [UIFont.boldSystemFont(ofSize: 15),UIFont.boldSystemFont(ofSize: 21)])
        }else{
            UIUtil.sl_setLabelAttributed(averageLabel, text: ["班级平均分：", "暂无"], fonts: [UIFont.boldSystemFont(ofSize: 15),UIFont.boldSystemFont(ofSize: 21)])
        }
        
        UIUtil.sl_setLabelAttributed(leftCommont.textLabel, text: ["表扬\n\n", "\(model.praiseScore ?? 0)"], fonts: [UIFont.boldSystemFont(ofSize: 14),UIFont.boldSystemFont(ofSize: 21)])
        UIUtil.sl_setLabelAttributed(rightCommont.textLabel, text: ["待改进\n\n", "\(model.improveScore ?? 0)"], fonts: [UIFont.boldSystemFont(ofSize: 14),UIFont.boldSystemFont(ofSize: 21)])
        progressView.setProgress(model.praiseScore ?? 0, model.improveScore ?? 0)
        UIUtil.sl_setLabelAttributed(countLabel, text: ["\(model.score ?? 0)\n","\(model.dateText)得分"], fonts: [UIFont.boldSystemFont(ofSize: 30),UIFont.boldSystemFont(ofSize: 15)])
    ...
    lazy var labelImageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_classstar_teacher_class_rectangle"))
        return imageView
    }()
