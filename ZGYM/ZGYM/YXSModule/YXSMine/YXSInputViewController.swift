...
class YXSInputViewController: YXSBaseViewController {

    var completionHandler: ((_ text:String, _ vc:YXSInputViewController)->())?
    private var maxLength:Int = 11
    
    init(maxLength:Int, completionHandler:(((_ text:String, _ vc:YXSInputViewController)->()))?) {
        super.init()
...
//        }
    ...
    lazy var tfText: YXSQSTextField = {
        let tf = YXSQSTextField()
        tf.mixedBackgroundColor = MixedColor(normal: UIColor.white, night: kNightForegroundColor)//yxs_hexToAdecimalColor(hex: "#F2F5F9")
