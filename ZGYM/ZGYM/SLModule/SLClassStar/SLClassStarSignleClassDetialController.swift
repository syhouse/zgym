...
class SLClassStarSignleClassDetialController: SLClassStarSignleClassCommonController {
    ...
    @objc func loadClassTopData() {
        SLEducationClassStarTeacherClassTopRequest.init(classId: classId, dateType: classModel?.dateType ?? DateType.W).requestCollection({ (classs:[SLClassStartClassModel]) in
    ...
    func loadBestData(){
        SLEducationClassStarChildrenBestRequest.init(classId: classId, dateType: classModel?.dateType ?? DateType.W).request({ (topModel:ClassStarBestModel) in
    ...
    func loadListData(){
        SLEducationClassStarTeacherEvaluationHistoryListPageRequest.init(classId: classId, currentPage: curruntPage,dateType: classModel?.dateType ?? DateType.W).requestCollection({ (list:[SLClassStarHistoryModel]) in
...
extension SLClassStarSignleClassDetialController{
    override func sl_user_routerEventWithName(eventName: String, info: [String : Any]?) {
        ...
        case KClassStarTeacherSingleFooterCommentEvent:
            publishClick()
        case kYMClassStarTeacherDetialHeaderViewLookDetialEvent:
