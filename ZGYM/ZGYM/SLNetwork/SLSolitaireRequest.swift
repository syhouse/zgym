...
//  ZGYM
//
//  Created by hnsl_mac on 2019/12/14.
...
class SLEducationCensusCensusDetailRequest: SLBaseRequset {
...
// MARK: - 参与人员列表（家长）
class SLEducationCensusParentStaffListRequest: SLBaseRequset {
    //接龙状态（0 全部 10 正在接龙 100 已结束:按照接龙的结束时间 ）
    init(censusId: Int) {
        super.init()
        host = homeHost
...
// MARK: - 提交(家长)
class SLEducationCensusParentCommitRequest: SLBaseRequset {
    //接龙状态（0 全部 10 正在接龙 100 已结束:按照接龙的结束时间 ）
...
// MARK: - 参与人员列表（老师）
class SLEducationCensusTeacherStaffListRequest: SLBaseRequset {
    //接龙状态（0 全部 10 正在接龙 100 已结束:按照接龙的结束时间 ）
    init(censusId: Int) {
        super.init()
        host = homeHost
...
class SLEducationCensusTeacherPublishRequest: SLBaseRequset {
...
class SLEducationCensusTeacherCensusListRequest: SLBaseRequset {
    //接龙状态（0 全部 10 正在接龙 100 已结束:按照接龙的结束时间 ）
...
class SLEducationCensusParentCensusListRequest: SLBaseRequset {
...
class SLEducationCensusTeacherUndoRequest: SLBaseRequset {
    init(censusId: Int) {
        super.init()
        method = .post
        host = homeHost
...
class SLEducationCensusUpdateTopRequest: SLBaseRequset {
    //    0:取消置顶|1:置顶
...
class SLEducationCensusTeacherTodoTodoRequest: SLBaseRequset {
...
class SLEducationCensusParentTodoRequest: SLBaseRequset {
    init(currentPage: Int,pageSize: Int = kPageSize) {
        super.init()
        method = .post
        host = homeHost
...
class SLEducationGradeFindNumberOfStudentsRequest: SLBaseRequset {
    //接龙状态（0 全部 10 未接 20 正在接龙 100 已结束:按照接龙的结束时间 ）
    init(gradeId: Int) {
        super.init()
        host = homeHost
