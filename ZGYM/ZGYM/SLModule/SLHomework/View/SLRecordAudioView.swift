...
class SLRecordAudioView: UIView {
    @discardableResult static func showRecord(audio:SLAudioModel? = nil, complete:((_ model: SLAudioModel) ->())? = nil) -> SLRecordAudioView{
        let view = SLRecordAudioView.init(audio: audio)
        view.complete = complete
        view.beginAnimation()
        return view
    }
    
    public var sourceDirectory: SLCacheDirectory = .HomeWork
    ...
    private var timerTool = SLCountDownTool()
    ...
    private func setRedayStartUI(){
        ...
        recordTotalLabel.mixedTextColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), night: kNightBCC6D4)
    ...
    private func setRecordingUI(){
        ...
        recordTotalLabel.mixedTextColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#575A60"), night: kNightBCC6D4)
    ...
    private func startRecord(){
        ...
        //1.获取cache文件地址
        filePathCaf = NSUtil.sl_recordCachePath(.HomeWork, "Record\(1).caf")
        filePathMp3 = NSUtil.sl_recordCachePath(.HomeWork, "Record\(1).mp3")
        ...
        timerTool.sl_startKeepTime { [weak self](time) in
            ...
            DispatchQueue.main.async {
                ...
                if time >= 60{
                    MBProgressHUD.sl_showMessage(message: "录音最大时长为1分钟")
    ...
    private func stopRecord(){
        ...
        //        SLConvertAudioFile.sharedInstance()?.conventToMp3(withCafFilePath: filePathCaf, mp3FilePath: filePathMp3, sampleRate: Int32(11025.0)){ (result) in
        //
        //        }
        SLConvertAudioFile.conventToMp3(withCafFilePath: filePathCaf, mp3FilePath: filePathMp3, sampleRate: Int32(8000)){ (result) in
            if result{
    ...
    private func play() {
        //            var path
                    ...
                    SLSSAudioPlayer.sharedInstance.play(url: playerUrl, loop: 1) {
                        [weak self] in
                        guard let strongSelf = self else { return }
    ...
    @objc private func certainClick(){
        if audioModel.time < 1{
            MBProgressHUD.sl_showMessage(message: "语音录制过短,请重录")
            return
        }
        complete?(audioModel)
        dismiss()
    ...
    // MARK: -getter
    lazy var recordTotalLabel: SLLabel = {
        let label = SLLabel()
    ...
    lazy var recordTipsLabel: SLLabel = {
        let label = SLLabel()
        label.font = UIFont.systemFont(ofSize: 15)
        label.mixedTextColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), night: kNightBCC6D4)
        label.text = "点击开始录音"
    ...
    lazy var refreshRecordButton: SLCustomImageControl = {
        let refreshRecordButton = SLCustomImageControl.init(imageSize: CGSize.init(width: 26, height: 26), position: .top, padding: 9.5)
        refreshRecordButton.font = UIFont.systemFont(ofSize: 16)
        refreshRecordButton.mixedTextColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), night: kNightBCC6D4)
        refreshRecordButton.title = "重录"
        refreshRecordButton.locailImage = "sl_publish_audio_refresh"
    ...
    lazy var cancelButton: SLButton = {
        let cancelButton = SLButton()
        cancelButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 15)
        cancelButton.setTitle("取消", for: .normal)
        cancelButton.setMixedTitleColor(MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#575A60"), night: UIColor.white), forState: .normal)
...
class RecordPayerControl: UIControl{
    ...
    lazy var imageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_publish_player"))
        return imageView
    }()
    
    lazy var aniLayer: CALayer = {
        let aniLayer = CAShapeLayer()
        aniLayer.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#A8C0FE").cgColor
    ...
    override func draw(_ rect: CGRect) {
        ...
        UIColor.sl_hexToAdecimalColor(hex: "#EDF2FF").set()
        //绘制路径
        //镂空型
        path.stroke()
        switch status {
        case .Recording:
            ...
            aniLayer.sl_circleCircleAnimal(size: rect.size)
            imageView.image = UIImage.init(named: "sl_publish_stop")
        case .redayRecord:
            imageView.image = UIImage.init(named: "sl_publish_player")
        case .Showing:
            aniLayer.isHidden = false
            aniLayer.sl_circleCircleAnimal(size: rect.size)
            imageView.image = UIImage.init(named: "sl_publish_stop")
