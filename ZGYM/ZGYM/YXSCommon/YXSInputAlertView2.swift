...
/// 带取消/确认按钮的输入框（按钮在同一排）
class YXSInputAlertView2: UIView {

    var complete: ((_ text: String, _ clickButton:YXSButton)->())?
    private var maxLength:Int = 11
    
    static func showIn(target: UIView, maxLength:Int = 11, complete:((_ text: String, _ clickButton:YXSButton) ->())?) -> YXSInputAlertView2{
        let view = YXSInputAlertView2()
