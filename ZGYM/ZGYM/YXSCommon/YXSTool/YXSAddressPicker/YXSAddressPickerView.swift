...
class YXSAddressPickerView: YXSBasePopingView, UIPickerViewDelegate, UIPickerViewDataSource {
    
    var selectedProvinceIndex: Int = 0
    var selectedCityIndex: Int = 0
    var selectedDistrictIndex: Int = 0
    var address: [SLAddressProvince] = YXSAddressPickerModel.shareInstance().address
    var block: ((_ province:String, _ city:String, _ district:String, _ view: YXSAddressPickerView)->())?
    
    init(frame: CGRect, completionHandler:((_ province:String, _ city:String, _ district:String, _ view: YXSAddressPickerView)->())?) {
    ...
    func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        let pickerLabel = YXSLabel()
        pickerLabel.adjustsFontSizeToFitWidth = true
        pickerLabel.textAlignment = .center
        pickerLabel.backgroundColor = UIColor.clear
        pickerLabel.font = UIFont.systemFont(ofSize: 14)
        pickerLabel.text = getTitle(pickerView: pickerView, forRow: row, forComponent: component)
        pickerLabel.mixedTextColor = MixedColor(normal: UIColor.yxs_hexToAdecimalColor(hex: "#898F9A"), night: kNightBCC6D4)
        return pickerLabel;
    }
