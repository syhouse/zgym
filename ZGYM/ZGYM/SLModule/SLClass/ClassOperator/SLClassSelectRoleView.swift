...
class SLClassSelectRoleView: UIView {
    
    /// 展示
    /// - Parameter complete: 点击所有按钮回调
    @discardableResult static func showAlert(complete:(( _ isSelectTeacher: Bool) ->())?) -> SLClassSelectRoleView{
        let view = SLClassSelectRoleView()
    ...
    lazy var teacherButton: SLButton = {
        let button = SLButton()
        ...
        button.sl_addLine(color: UIColor.sl_hexToAdecimalColor(hex: "#F3F5F9"), lineHeight: 1)
        return button
    }()
    
    lazy var parentButton: SLButton = {
        let button = SLButton()
