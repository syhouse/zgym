...
class SLPhotoShowAlubmCell: UICollectionViewCell {
    ...
    func setCellModel(_ model: SLPhotoAlbumsDetailListModel){
        ...
        if model.resourceType == 1{
            imageView.sd_setImage(with: URL.init(string: model.bgUrl ?? ""), placeholderImage: UIImage.init(named: "sl_photo_nocover"))
        }else{
            imageView.sd_setImage(with: URL.init(string: model.resourceUrl ?? ""), placeholderImage: UIImage.init(named: "sl_photo_nocover"))
        }
    }
    ...
    lazy var playerButton: SLButton = {
        let btn = SLButton()
        btn.addTarget(self, action: #selector(playerClick), for: .touchUpInside)
        btn.setBackgroundImage(UIImage(named: "sl_publish_play"), for: .normal)
        btn.isHidden = true
        return btn
    }()
}
