...
class SLFriendDragListView: UIView {
    ...
    var itemListH: CGFloat {
        get {
            ...
            return items.last!.sl_bottom + itemMargin
        }
    }
    /**
     *  item 最多个数 默认9个
    ...
    @objc func pan(_ pan: UIPanGestureRecognizer?) {
        ...
        if pan?.state == UIGestureRecognizer.State.began {
            ...
            tagButton.sl_top = rect.minY + tagButton.sl_top
            tagButton.sl_left = rect.minX + tagButton.sl_left
        }
        ...
        if pan?.state == UIGestureRecognizer.State.ended{
            ...
            UIView.animate(withDuration: 0.25, animations: {
                ...
                tagButton.sl_left += rect.minX
                tagButton.sl_top += rect.minY
            }) { (finish) in
                if finish{
    ...
    lazy var deleteView: UIView = {
        ...
        let button = SLButton(type: .custom)
        button.tag = 201809
        ...
        button.sl_setIconInLeftWithSpacing(25)
