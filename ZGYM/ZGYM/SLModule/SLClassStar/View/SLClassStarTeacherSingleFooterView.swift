...
class SLClassStarTeacherSingleFooterView: UITableViewHeaderFooterView {
    ...
    func setFooterModel(_ model: SLClassStartClassModel?){
        if let model = model{
            ...
            UIUtil.sl_setLabelAttributed(commentLabel, text: ["全国已有", "200万", "名学生正在使用此功能，班级之星是学生德育培养的重要辅助功能，亲爱的老师赶快去使用吧！"], colors: [UIColor.sl_hexToAdecimalColor(hex: "#BABDC2"),kBlueColor, UIColor.sl_hexToAdecimalColor(hex: "#BABDC2")])
        }
    }
    ...
    @objc func goCommont(){
        next?.sl_routerEventWithName(eventName: KClassStarTeacherSingleFooterCommentEvent)
    }
    ...
    lazy var goCommontButton: SLButton = {
        let goCommontButton = SLButton()
        ...
        goCommontButton.sl_gradualBackground(frame: CGRect(x: 0, y: 0, width: 192, height: 49), startColor: UIColor.sl_hexToAdecimalColor(hex: "#4B73F6"), endColor: UIColor.sl_hexToAdecimalColor(hex: "#77A3F8"), cornerRadius: 24.5)
        goCommontButton.sl_shadow(frame: CGRect(x: 0, y: 0, width: 192, height: 49), color: UIColor.sl_hexToAdecimalColor(hex: "#4C74F6"), cornerRadius: 7.5, offset: CGSize(width: 0, height: 3))
