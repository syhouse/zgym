...
protocol SLSelectMediaHelperDelegate {
...
extension SLSelectMediaHelperDelegate {
    func didSelectMedia(asset: SLMediaModel){}

    func didSelectSourceAssets(assets: [SLMediaModel]){}
}

class SLSelectMediaHelper: NSObject,TZImagePickerControllerDelegate {
    static let shareHelper = SLSelectMediaHelper()
    ...
    var delegate: SLSelectMediaHelperDelegate!
    var maxcount: Int = 1
    ...
    public func showSelectMedia(selectImage: Bool = false, selectVedio: Bool = false,selectAll: Bool = false, customVideo: Bool = false, maxCount:Int = 1, customVideoFinish: ((SLMediaModel) -> Void)? = nil) {
    ...
    private func pushRecordVideo(complete: ((SLMediaModel) -> Void)?){
        let vc = SLRecordVideoController.init(complete:complete)
    ...
    func imagePickerController(_ picker: TZImagePickerController!, didFinishPickingPhotos photos: [UIImage]!, sourceAssets assets: [Any]!, isSelectOriginalPhoto: Bool) {
        ...
        delegate?.didSelectSourceAssets(assets: SLMediaModel.getMediaModels(assets: assets as! [PHAsset]))
    }
    
    func imagePickerController(_ picker: TZImagePickerController!, didFinishPickingVideo coverImage: UIImage!, sourceAssets asset: PHAsset!) {
//        大小50M以内 时长100s
        if asset.duration > 100{
            MBProgressHUD.sl_showMessage(message: "视频时长不能超过100s")
            return
        }
        ...
        if size > videoMax{
            MBProgressHUD.sl_showMessage(message: "视频大小不能超过50M")
            return
        }
...
extension SLSelectMediaHelper: (UIImagePickerControllerDelegate & UINavigationControllerDelegate){
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if (type == "public.image") {
            ...
            if let image = image{
                ...
                MBProgressHUD.sl_showMessage(message: "拍摄失败")
            }
