...
class SLPhotoCreateAlbumController: SLEditAlbumBaseController {
    ...
    lazy var createBtn: SLButton = {
        let btn = SLButton()
        btn.setTitle("完成新建", for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 18)
        btn.setMixedTitleColor(MixedColor(normal: 0xFFFFFF, night: 0xFFFFFF), forState: .normal)
        btn.sl_gradualBackground(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH - 57, height: 49), startColor: UIColor.sl_hexToAdecimalColor(hex: "#4B73F6"), endColor: UIColor.sl_hexToAdecimalColor(hex: "#77A3F8"), cornerRadius: 24.5)
        btn.sl_shadow(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH - 57, height: 49), color: UIColor.gray, cornerRadius: 24.5, offset: CGSize(width: 4, height: 4))
