...
enum SLOperationPosition{
    case home//首页操作
    case friendCircle//班级圈首页操作
    case singleHome//单个班级首页操作
    case list//列表页操作
    case detial//详情页操作
}
...
private class SLEducationUserDetailRequest: SLBaseRequset {
    override init() {
        super.init()
        method = .post
        path = userDetail
...
extension UIUtil{
    ...
    public static func sl_setLabelParagraphText(_ label: UILabel?, text: String?,font: UIFont = kTextMainBodyFont, lineSpacing: CGFloat = kMainContentLineHeight, removeSpace: Bool = false) {
        var newText = text
        if removeSpace{
    ...
    public static func sl_setLabelAttributed(_ label: UILabel?, text texts: [String]?, colors: [UIColor]? = nil, fonts: [UIFont]? = nil, paragraphLineSpacing: CGFloat? = nil) {
        ...
        sl_setLabelAttributed(label, text: texts, colors: mixedColor, fonts: fonts,paragraphLineSpacing: paragraphLineSpacing)
    }
    ...
    ///   - fonts: 文本字体数组
    ///   - paragraphLineSpacing: 段落间距
    public static func sl_setLabelAttributed(_ label: UILabel?, text texts: [String]?, colors: [MixedColor]?, fonts: [UIFont]? = nil, paragraphLineSpacing: CGFloat? = nil) {
    ...
    /// - Parameter font: 字体
    /// - Parameter width: 宽度
    public static func sl_getTextHeigh(textStr : String?, font : UIFont, width : CGFloat) -> CGFloat{
        let dic = NSDictionary(object: font, forKey : kCTFontAttributeName as! NSCopying)
        return sl_getTextHeigh(textStr: textStr, attributes:  dic as? [NSAttributedString.Key:Any], width: width)
    }
    ...
    /// - Parameter font: 字体
    /// - Parameter width: 宽度
    public static func sl_getTextHeigh(textStr : String?, attributes : [NSAttributedString.Key:Any]?, width : CGFloat) -> CGFloat{
        return sl_getTextHeigh(textStr: textStr, attributes:  attributes, width: width, numberOfLines: nil)
    }
    ...
    public static func sl_getTextHeigh(textStr : String?, attributes : [NSAttributedString.Key:Any]?, width : CGFloat, numberOfLines: Int?) -> CGFloat{
    ...
    ///   - font: 字体
    public static func sl_getTextField(_ edgeInsets: UIEdgeInsets = UIEdgeInsets.zero, placeholder: String? = nil, placeholderColor: UIColor? = nil, mixedTextColor: MixedColor? = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), night: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA")), font: UIFont? = UIFont.systemFont(ofSize: 16) ) -> SLQSTextField{
    ...
    ///设置虚线
    public static func sl_drawDashLine(_ lineView:UIView,strokeColor: UIColor, lineWidth: CGFloat = 1, lineLength: CGFloat = 10, lineSpacing: CGFloat = 5) {
...
extension UIUtil{
    ...
    static func sl_shareLink(requestModel: HMRequestShareModel,shareModel: SLShareModel, compelect: (()->())? = nil){
        MBProgressHUD.sl_showLoading()
        SLEducationShareLinkRequest.init(model: requestModel).request({ (json) in
    ...
    /// 孩子有多个家长 拨打电话
    static func sl_callPhoneNumberRequest(childrenId: Int, classId: Int) {
        MBProgressHUD.sl_showLoading()
        SLEducationTeacherParentsBaseInfoRequest(childrenId: childrenId, classId: classId).requestCollection({ (list:[SLParentModel]) in
    ...
    /// 孩子有多个家长 IM私聊
    static func sl_chatImRequest(childrenId: Int, classId: Int) {
        MBProgressHUD.sl_showLoading()
        SLEducationTeacherParentsBaseInfoRequest(childrenId: childrenId, classId: classId).requestCollection({ (list:[SLParentModel]) in
            //            guard let weakSelf = self else {return}
            MBProgressHUD.sl_hideHUD()
            ...
            SLSolitaireSelectReasonView(items: keys, selectedIndex: defaultIndex, title: "要联系\(list.first?.realName ?? "")的私聊吗？", inTarget: curruntNav().view) { (view, index) in
                ...
                UIUtil.TopViewController().sl_pushChatVC(imId: imId ?? "")
                view.cancelClick()
            }
    ...
    ///   - childrenId: 孩子id
    ///   - classId:    班级id
    static func sl_loadClassStartReminderRequest(teacherLists: [SLClassStartTeacherModel],childrenId: Int,classId: Int){
        ...
        if teacherIdList.count == 0{
            MBProgressHUD.sl_showMessage(message: "当前没有可提醒的老师")
            return
        }
        SLEducationParentOneTouchReminderRequest.init(teacherIdList: teacherIdList, opFlag: 1, serviceId: 0, serviceType: 6,childrenId: childrenId, classId: classId).request({ (json) in
            MBProgressHUD.sl_showMessage(message: "提醒成功")
    ...
    /// 阅读
    /// - Parameter model: SLHomeListModel
    static func sl_loadReadData(_ model: SLHomeListModel,showLoading: Bool = true){
        UIUtil.sl_reduceHomeRed(serviceId: model.serviceId ?? 0, childId: model.childrenId ?? 0)
        if SLPersonDataModel.sharePerson.personRole == .TEACHER || model.isRead == 1{
            return
        }
        model.isRead = 1
        if model.type == .homework || model.type == .notice{
            if model.type == .homework{
                SLEducationHomeworkCustodianUpdateReadRequest.init(childrenId: model.childrenId ?? 0, homeworkCreateTime: model.createTime ?? "", homeworkId: model.serviceId ?? 0).request(nil, failureHandler: nil)
            }else if model.type == .notice{
                SLEducationNoticeCustodianUpdateReadRequest.init( childrenId: model.childrenId ?? 0, noticeCreateTime: model.createTime ?? "", noticeId: model.serviceId ?? 0).request(nil, failureHandler: nil)
            }
            if showLoading{
                MBProgressHUD.sl_showMessage(message: "已阅读", inView: UIUtil.curruntNav().view)
            }
    ...
    ///   - positon: 操作位置
    ///   - complete: 撤销成功block
    static func sl_loadRecallData(_ model: SLHomeListModel,positon: SLOperationPosition = .home, complete:(() -> ())?){
        sl_loadRecallData(model.type, id: model.serviceId ?? 0, createTime: model.createTime ?? "",positon: positon, complete: complete)
    }
    ...
    ///   - positon: 操作位置
    ///   - complete: 撤销成功block
    static func sl_loadRecallData(_ type: HomeType,id: Int = 0,createTime: String = "",positon: SLOperationPosition = .home, complete:(() -> ())?){
        let showTitle = type == HomeType.friendCicle ? "是否撤销该条优成长？" : "你是否确认撤销？"
        let sucessMessage = type == HomeType.friendCicle ? "删除成功" : "撤销成功"
        SLCommonAlertView.showAlert(title: showTitle, rightClick: {
            var requset: SLBaseRequset!
            switch type {
            case .homework:
                requset = SLEducationHomeworkCancelRequest.init( homeworkId: id,homeworkCreateTime: createTime)
            case .notice:
                requset = SLEducationNoticeCancelRequest.init( noticeId: id,noticeCreateTime: createTime)
            case .solitaire:
                requset = SLEducationCensusTeacherUndoRequest.init(censusId: id)
            case .punchCard:
                requset = SLEducationClockInTeacherUndoRequest.init(clockInId: id)
            case .friendCicle:
                requset = SLEducationClassCircleCancelRequest.init(classCircleId: id)
            default:
                break
            }
            MBProgressHUD.sl_showLoading()
            requset.request({ (result) in
                MBProgressHUD.sl_showMessage(message: sucessMessage)
                complete?()
                ...
                case .friendCircle:
                    if type == .friendCicle && SLPersonDataModel.sharePerson.personRole == .TEACHER && SLPersonDataModel.sharePerson.personStage == .KINDERGARTEN{
    ...
    static func sl_loadClassCircleMessageListData(completion:(() -> ())? = nil){
        SLEducationClassCircleMessageTipsRequest().request({ (result: SLFriendsTipsModel) in
            SLPersonDataModel.sharePerson.friendsTips = result
            if result.count ?? 0 > 0 {
                RootController().sl_showBadgeOnItem(index: 1, count: result.count!)
                completion?()
            }
        }, failureHandler: nil)
    }
    
    static func sl_loadUserDetailRequest(_ completion: ((_ model:SLEducationUserModel) -> ())?,
                                         failureHandler: ((String, String) -> ())?){
        sl_loadUserDetailRequest(completion, failureHandler: failureHandler,IMLoginSucess: nil)
    }
    
    static func sl_loadUserDetailRequest(_ completion: ((_ model:SLEducationUserModel) -> ())?,
                                         failureHandler: ((String, String) -> ())?,IMLoginSucess:(() -> ())? = nil){
        SLEducationUserDetailRequest().request({ (resultModel:SLEducationUserModel) in
            resultModel.accessToken = SLPersonDataModel.sharePerson.userModel.accessToken
            resultModel.hasSetPassword = SLPersonDataModel.sharePerson.userModel.hasSetPassword
            resultModel.gradeNum = SLPersonDataModel.sharePerson.userModel.gradeNum
            ...
            SLPersonDataModel.sharePerson.userModel = resultModel
            ...
            DispatchQueue.global().async {
                SLLocalMessageHelper.shareHelper.sl_changeListData()
            }
    ...
    ///   - positon: 操作位置
    ///   - sucess: 成功block
    static func sl_loadUpdateTopData(type: HomeType,id: Int = 0,createTime:String = "", isTop: Int = 0,positon: SLOperationPosition = .home, sucess: (()->())? = nil){
        MBProgressHUD.sl_showLoading()
        var request: SLBaseRequset?
        if type == .homework{
            request = SLEducationHomeworkUpdateTopRequest.init(homeworkId: id, homeworkCreateTime: createTime, isTop: isTop)
        }else if type == .notice{
            request = SLEducationNoticeUpdateTopRequest.init(noticeId: id, noticeCreateTime: createTime, isTop: isTop)
        }else if type == .punchCard{
            request = SLEducationClockInUpdateTopRequest.init(clockInId: id)
        }else if type == .solitaire{
            request = SLEducationCensusUpdateTopRequest.init(censusId: id,isTop: isTop)
        }
        request?.request({ (json) in
            MBProgressHUD.sl_showMessage(message: "\(isTop == 0 ? "取消置顶" : "置顶")成功")
            sucess?()
            switch positon {
            case .detial:
    ...
    static func sl_reduceAgenda(serviceId: Int,info: [String: Any]){
        if SLPersonDataModel.sharePerson.personRole == .TEACHER{//老师去掉待办红点
            SLEducationTodoUpdateRedPointRequest.init(serviceId: serviceId).request({ (result) in
    ...
    static func sl_reduceHomeRed(serviceId: Int, childId: Int){
        if SLPersonDataModel.sharePerson.personRole == .PARENT{
            if SLLocalMessageHelper.shareHelper.sl_isLocalMessage(serviceId: serviceId,childId: childId){
                SLLocalMessageHelper.shareHelper.sl_removeLocalMessage(serviceId: serviceId, childId: childId)
...
extension UIUtil{
    ...
    static func sl_changeFriendCirclePrise(_ friendCircleModel: SLFriendCircleModel,positon: SLOperationPosition = .home, reloadViewBlock:(() -> ())?){
        if SLPersonDataModel.sharePerson.isNetWorkingConnect == false{
            //            friendCircleModel.isOnRequsetPraise = false
            MBProgressHUD.sl_showMessage(message: "网络不给力，请检查网络")
            return
        }
        var praiseModel: SLFriendsPraiseModel! = nil
        ...
        if isCancelPraise {
            ...
            let result:SLFriendsPraiseModel = SLFriendsPraiseModel.init(JSON: ["":""])!
            result.classCircleId = friendCircleModel.classCircleId
            result.userType = SLPersonDataModel.sharePerson.personRole.rawValue
            result.userId = SLPersonDataModel.sharePerson.userModel.id
            if SLPersonDataModel.sharePerson.personRole == .TEACHER{
                result.userName = SLPersonDataModel.sharePerson.userModel.name
            }else{
                var childrenNames = [String]()
                var gradeIds = [Substring]()
                gradeIds = friendCircleModel.gradeId?.split(separator: ",") ?? [Substring]()
                for gradeId in gradeIds{
                    if let childrens  = SLPersonDataModel.sharePerson.userModel.children{
        ...
        SLEducationClassCirclePraiseUpdateRequest.init(classCircleId: friendCircleModel.classCircleId ?? 0).request({ (result:SLFriendsPraiseModel) in
    ...
    static func sl_changeCommentFriendCirclePrise(_ classCircleId: Int,commentId: Int? = nil,commentModel: SLFriendsCommentModel? = nil,content: String? = nil,positon: SLOperationPosition = .home, reloadViewBlock:((SLFriendsCommentModel) -> ())?){
        if SLPersonDataModel.sharePerson.isNetWorkingConnect == false{
            MBProgressHUD.sl_showMessage(message: "网络不给力，请检查网络")
            return
        }
        ...
        if let commentId = commentId{
            isAddComment = false
            request = SLEducationClassCircleCommentCancelRequest.init(classCircleId: classCircleId, id: commentId)
        }else{
            request = SLEducationClassCircleCommentSaveRequest.init(classCircleId: classCircleId, type: commentModel?.id == nil ? FriendCircleComment.COMMENT : FriendCircleComment.REPLY, content: content ?? "", id: commentModel?.id)
        }
...
extension UIUtil{
    ...
    public static func setCommonThemeConfig(){
        ...
        kImageUserIconPartentDefualtImage = NightNight.theme == .night ? UIImage.init(named: "sl_defult_partent_night") : UIImage.init(named: "sl_defult_partent")
        kImageUserIconTeacherDefualtImage = NightNight.theme == .night ? UIImage.init(named: "sl_defult_teacher_night") : UIImage.init(named: "sl_defult_teacher")
        kImageUserIconStudentDefualtImage = NightNight.theme == .night ? UIImage.init(named: "sl_defult_student_night") : UIImage.init(named: "sl_defult_student")
