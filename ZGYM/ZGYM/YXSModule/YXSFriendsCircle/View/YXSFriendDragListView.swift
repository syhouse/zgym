...
class YXSFriendDragListView: UIView {
     ...
     *  添加item
     * */
    func addItem(_ item: YXSFriendDragItem) {
        //移除“添加”item
        ...
        var tag = items.count
        var addItem: YXSFriendDragItem! = nil
     ...
     *  添加多个item
     */
    func addItems(_ items: [YXSFriendDragItem]) {
        if self.width == 0{
            SLLog("先设置标签列表的frame")
            return
        }
     ...
     *  点击标签，执行Block
     */
    var clickItemBlock: ((YXSFriendDragItem) -> Void)?
    /**
     *  移除回调
     */
    var deleteItemBlock: ((YXSFriendDragItem) -> Void)?
    
    // MARK: -privite
    private var items:[YXSFriendDragItem] = [YXSFriendDragItem]()
     ...
     *  获取所有item
     */
    public func itemArray() ->[YXSFriendDragItem] {
        return items
    }
    ...
    @objc func clickItem(_ tap: UITapGestureRecognizer?) {
        ...
        dismissKeyBord()
        clickItemBlock?(tap?.view as! YXSFriendDragItem)
    }
    ...
    @objc func pan(_ pan: UIPanGestureRecognizer?) {
        ...
        let tagButton:YXSFriendDragItem = pan?.view as! YXSFriendDragItem
    ...
    func buttonCenterInButtons(_ curItem: YXSFriendDragItem) -> YXSFriendDragItem?{
     ...
     *  删除item
     */
    func delete(_ item: YXSFriendDragItem) {
    ...
    // 计算标签按钮frame（按规律排布）
    func setupItemButtonRegularFrame(_ tagItem: YXSFriendDragItem?) {
        // 获取角标
        let i = tagItem?.tag ?? 0
