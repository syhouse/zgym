...
class SLCommonPublishView: UIView{
    // MARK: - public
    /// 输入内容时偏移更新
    ...
    private var publishModel: SLPublishModel
    ...
    // MARK: - init
    init(publishModel: SLPublishModel,isShowMedia: Bool = true,limitTextLength: Int = 1000,audioMaxCount: Int = 1) {
        ...
        if isShowMedia{
            ...
            if publishModel.audioModels.count != 0{
                for index in 0..<publishModel.audioModels.count{
                    let voiceView = viewWithTag(index + kAudioViewOrginTag) as? SLVoiceView
    ...
    func showSelectMedia(_ isAdd: Bool){
        ...
        if isAdd{
            SLSelectMediaHelper.shareHelper.pushImagePickerController(mediaStyle: .onlyImage,maxCount: count)
        }else{
            SLSelectMediaHelper.shareHelper.pushImagePickerController(assets,mediaStyle: .onlyImage,maxCount: maxcount - serviceMedias.count)
        }
        
        SLSelectMediaHelper.shareHelper.delegate = self
    }
    
    func showAudio(_ model:SLAudioModel? = nil,index: Int? = nil){
    ...
    func addSingleMedia(asset: SLMediaModel){
    ...
    // MARK: - action
    func dealButtonViewEvent(_ event: PublishViewButtonEvent)
    {
        ...
        case .link:
            SLHomeworkInputLinkAlert.showAlert(publishModel.publishLink) { [weak self](link) in
                guard let strongSelf = self else { return }
                strongSelf.publishModel.publishLink = link
                strongSelf.updateUI()
            }
        case .vedio:
            let vc = SLRecordVideoController.init(complete: { [weak self](asset) in
                guard let strongSelf = self else { return }
    ...
    lazy var textCountlabel: SLLabel = {
        let label = SLLabel()
    ...
    lazy var buttonView: SLPublishViewButtonView = {
...
// MARK: - SLSelectMediaHelperDelegate
extension SLCommonPublishView: SLSelectMediaHelperDelegate{
...
extension SLCommonPublishView{
    /// 创建 SLFriendDragItem
    /// - Parameter model: 本地资源
    /// - Parameter isAdd: 是否是添加按钮
    func getItem(_ model: SLMediaModel? = nil, isAdd: Bool = false) -> SLFriendDragItem{
