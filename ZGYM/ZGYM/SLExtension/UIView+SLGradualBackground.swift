...
extension UIView {
    /// 设置渐变背景颜色
    func sl_gradualBackground(frame: CGRect, startColor: UIColor, endColor: UIColor, cornerRadius: CGFloat) {
    ...
    func sl_shadow(frame: CGRect, color: UIColor = UIColor(red: 0.3, green: 0.45, blue: 0.96, alpha: 0.5), cornerRadius: CGFloat, offset: CGSize = CGSize(width: 0, height: 3)) {
        sl_shadow(frame: frame, color: MixedColor(normal: color, night: color), cornerRadius: cornerRadius, offset: offset)
    }
    
    /// 添加阴影 MixedColor
        func sl_shadow(frame: CGRect, color: MixedColor, cornerRadius: CGFloat, offset: CGSize = CGSize(width: 0, height: 3)) {
            var shadow: CALayer!
    ...
    /**
     For Example
         ...
         SLButton *button = [[SLButton alloc] init];
