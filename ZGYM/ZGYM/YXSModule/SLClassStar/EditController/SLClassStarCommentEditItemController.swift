...
class SLClassStarCommentEditItemController: YXSBaseViewController {
    var compeletBlock: (() -> ())?
    ...
    // MARK: -loadData
    func loadData(){
        YXSEducationFEvaluationListTypeListRequest(stage: stage).requestCollection({ (list:[SLClassStarTypeModel]) in
