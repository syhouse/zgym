...
class SLChatHelper: NSObject, TIMMessageListener, TIMUserStatusListener{
    var onUserStatusBlock: (()->())?
    /// 防重复从服务器拿新Usersig
    var isUserSigRequestEnd: Bool = true
    
    @objc static let sharedInstance: SLChatHelper = {
        let instance = SLChatHelper()
        // setup code
        return instance
    }()
    ...
    @objc func login(completionHandler:(()->())? = nil) {
        ...
        if SLPersonDataModel.sharePerson.userModel.type == nil || SLPersonDataModel.sharePerson.userModel.type?.count ?? 0 == 0 {
//            MBProgressHUD.sl_showMessage(message: "未获取到用户ID")
            return
        }
        ...
        userSigRequest {[weak self](userSig) in
            ...
            let identifier = "\(SLPersonDataModel.sharePerson.userModel.id ?? 0)\(SLPersonDataModel.sharePerson.personRole.rawValue)"
...
//                MBProgressHUD.sl_showMessage(message: "\(msg ?? ""):\(code)")
            })
        }
    ...
    @objc func userSigRequest(completionHandler:@escaping((_ userSig: String)->())) {
        ...
        SLEducationTencentImUserSignRequest().request({ [weak self](json) in
            guard let weakSelf = self else {return}
            ...
            guard let weakSelf = self else {return}
            weakSelf.isUserSigRequestEnd = true
            MBProgressHUD.sl_showMessage(message: "Sig请求出错:\(msg)")
        }
    }
    ...
    @objc func refreshData() {
        ...
        self.sl_showBadgeOnItem(index: 2, count: unReadCount)
    }
...
//                weakSelf.registAppleDeviceToken()
    ...
    func onNewMessage(_ msgs: [Any]!) {
        ...
        for sub:TIMMessage in arr {
            ...
            if isServiceMessage(msg: sub) {
                ...
                if model.ring == 1 {
                    /// 播放音效
                    SLSSAudioPlayer.sharedInstance.playMessageSoundEffect()
                }
    ...
    @objc func onUserStatus(notification: NSNotification) {
        ...
        case Int(TUIUserStatus.TUser_Status_ForceOffline.rawValue):
            if SLPersonDataModel.sharePerson.isLogin {
                SLPersonDataModel.sharePerson.userLogout()
            }
            let alert = SLConfirmationAlertView.showIn(target: UIUtil.RootController().view, hasCancel: false) { (sender, view) in
                view.close()
            }
    ...
    @objc func refreshApplicationIconBadgeNum() -> Int32 {
        if isLogin() {
            ...
            if SLPersonDataModel.sharePerson.personRole == .PARENT{
                unReadCount += SLLocalMessageHelper.shareHelper.sl_localMessageCount()
            }
