...
class SLClassStarCommentEditItemListController: SLBaseViewController, JXCategoryViewDelegate, JXCategoryListContainerViewDelegate {
    var defultIndex: Int
    ...
    // MARK: - Request
    func refreshData() {
        SLEducationHomeworkQueryCommitReadInfoRequest(gradeId: detailModel?.classId ?? 0, homeworkCreateTime: detailModel?.createTime ?? "", homeworkId: detailModel?.serviceId ?? 0).request({ (json) in
    ...
    lazy var categoryView: JXCategoryTitleView = {
        ...
        view.titleColor = NightNight.theme == .night ? kNight898F9A : UIColor.sl_hexToAdecimalColor(hex: "#575A60")
...
extension SLClassStarCommentEditItemListController: SLRouterEventProtocol{
