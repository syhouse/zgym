...
class YXSAddClassAlertView: UIView {
    
    static func showIn(target: UIView, complete:((_ type: ClassEvent) ->())?) {
        let view = YXSAddClassAlertView()
    ...
    // MARK: - Action
    @objc func createClassClick(sender: YXSButton) {
    ...
    // MARK: - LazyLoad
    lazy var btnCreateClass: YXSButton = {
        let btn = YXSButton()
    ...
    lazy var btnAddClass: YXSButton = {
        let btn = YXSButton()
