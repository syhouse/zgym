...
class SLClassRepeatController: SLBaseViewController {
    ...
    // MARK: -action
    @objc func isClick(){
        let vc = SLClassRepeatCheckController.init(errorModel: errorModel)
        vc.cheakSucess = {
            [weak self] in
    ...
    lazy var isMyChildButton: SLButton = {
        let btn = SLButton()
    ...
    lazy var notMyChildButton: SLButton = {
        let btn = SLButton()
