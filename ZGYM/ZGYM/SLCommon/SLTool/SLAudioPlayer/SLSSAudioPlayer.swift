...
//  ZGYM
//
//  Created by sl_mac on 2019/12/18.
//  Copyright © 2019 hnsl_mac. All rights reserved.
//

import UIKit

class SLSSAudioPlayer: NSObject {
    static let sharedInstance: SLSSAudioPlayer = {
        let instance = SLSSAudioPlayer()
        // setup code
        return instance
    }()
...
//        }
        ...
        //下载音频资源
        if SLDownloaderHelper.helper.hasDownloadSucess(url: url.absoluteString){
            localUrl = URL.init(fileURLWithPath: SLDownloaderHelper.helper.downloadPath(url: url.absoluteString))
        }else{
            SLDownloaderHelper.helper.downloadFile(urlStr: url.absoluteString)
        }
