//
//  SLClassAddInfoController.swift
//  ZGYM
//
...
class SLClassAddInfoController: SLBaseViewController {
    ...
    ///   - gradeId: 班级id
    ///   - childModel:   孩子model
    init(_ isTeacherRole: Bool, gradeId: Int, stage: StageType,childModel: SLChildrenModel? = nil) {
    ...
    func loadJoinRequest(){
        ...
        SLEducationGradeMemberJoinRequest.init(gradeId: gradeId, position: isTeacherRole ? PersonRole.TEACHER : PersonRole.PARENT,subject: selectSubject?.paramsKey ?? "",childrenId: childModel?.id, relationship: selectRelationship?.paramsKey ?? "", realName: studentNameField.text!, studentId: studentNumberField.text!,avatar: headerUrl).request({ (result) in
            ...
            MBProgressHUD.hide(for: self.view, animated: true)
            let errors = [SLClassAddErrorModel].init(JSONString: msg)
            if let errors = errors{
                ...
                if let errorModel = errorModel{
                    if errorModel.account != nil{
                        ...
                        let vc = SLClassRepeatController.init(errorModel: errorModel)
                        vc.cheakReuslt = {
                            [weak self](sucess: Bool) in
                            guard let strongSelf = self else { return }
                            if sucess{//同一孩子  提交孩子
                                let chidlModel = SLChildrenModel.init(JSON: ["" : ""])
                                ...
                                if errorModel.relationships?.contains(strongSelf.selectRelationship?.paramsKey ?? "") ?? false{
                                    MBProgressHUD.sl_showMessage(message: "请重新选择身份")
                                }else{
    ...
    var selectRelationship: SLSelectSectionModel!
    @objc func roleSelectClick(){
        SLSelectAlertView.showAlert(Relationships, selectSubject,title: "选择关系") { [weak self](model) in
            guard let strongSelf = self else { return }
    ...
    @objc func requestUploadImage(asset: SLMediaModel) {
        ...
        SLUploadSourceHelper().uploadImage(mediaModel: asset, sucess: {(successUrl) in
            MBProgressHUD.sl_hideHUDInView(view: self.view)
            self.headerUrl = successUrl
            self.avarSection.imgAvatar.image = asset.showImg
        }) { (msg, code) in
    ...
    // MARK: - getter&setter
    lazy var studentNameLabel: SLLabel = {
        let label = SLLabel()
    ...
    lazy var studentNumberLabel: SLLabel = {
        let label = SLLabel()
        label.font = UIFont.systemFont(ofSize: 14)
        label.textColor = UIColor.sl_hexToAdecimalColor(hex: "#898F9A")
        label.text = "学号"
    ...
    lazy var studentNameField: SLQSTextField = {
        let classField = UIUtil.sl_getTextField(UIEdgeInsets.init(top: 0, left: 15, bottom: 0, right: 0), placeholder: "请输入真实姓名", placeholderColor: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), mixedTextColor:MixedColor(normal: kTextMainBodyColor, night: UIColor.white))
    ...
    lazy var studentNumberField: SLQSTextField = {
        let classField = UIUtil.sl_getTextField(UIEdgeInsets.init(top: 0, left: 15, bottom: 0, right: 0), placeholder: "请输入孩子学号", placeholderColor: UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA"), mixedTextColor:MixedColor(normal: kTextMainBodyColor, night: UIColor.white))
        classField.mixedBackgroundColor = MixedColor(normal: UIColor.white, night: kNightForegroundColor)
        classField.addTarget(self, action: #selector(textChange), for: .editingChanged)
        return classField
    }()
    ...
    lazy var joinClassButton: SLButton = {
        
        let joinClassButton = SLButton.init(frame: CGRect.init(x: 0, y: 0, width: 230, height: 49))
        ...
        joinClassButton.sl_gradualBackground(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH - 57, height: 49), startColor: UIColor.sl_hexToAdecimalColor(hex: "#4B73F6"), endColor: UIColor.sl_hexToAdecimalColor(hex: "#77A3F8"), cornerRadius: 24.5)
        joinClassButton.sl_shadow(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH - 57, height: 49), color: UIColor(red: 0.3, green: 0.45, blue: 0.96, alpha: 0.5), cornerRadius:  24.5, offset: CGSize(width: 0, height: 3))
...
extension SLClassAddInfoController: SLSelectMediaHelperDelegate{
