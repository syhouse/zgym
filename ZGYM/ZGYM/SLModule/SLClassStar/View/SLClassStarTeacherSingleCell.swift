...
class SLClassStarTeacherSingleCell: UITableViewCell {
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        sl_addLine(position: .bottom, mixedBackgroundColor: MixedColor(normal: kLineColor, night: kLineColor), leftMargin: 71, rightMargin: 15.5)
    ...
    var isLastRow: Bool = false{
        didSet{
            if isLastRow{
                bgView.sl_addRoundedCorners(corners: [.bottomLeft,.bottomRight], radii: CGSize.init(width: 4, height: 4), rect: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH - 41, height: 69))
            }else{
                bgView.sl_addRoundedCorners(corners: [.bottomLeft,.bottomRight], radii: CGSize.init(width: 0, height: 0), rect: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH - 41, height: 69))
            }
        }
    }
    ...
    func sl_setCellModel(_ model: SLClassStarHistoryModel){
        self.model = model
        ...
        detialLabel.text = "\(Date.sl_Time(dateStr: model.createTime ?? "")) 由\(model.teacherName ?? "") 点评 \(showChildName ? model.childrenName ?? "" : "")"
    }
    // MARK: -getter&setter
...
//        kNightForegroundColor
        ...
        sl_addLine(position: .bottom, mixedBackgroundColor: MixedColor(normal: kLineColor, night: kLineColor), leftMargin: 15)
    ...
    func sl_setCellModel(_ model: SLClassStartTeacherModel){
    ...
    lazy var remindButton: SLButton = {
