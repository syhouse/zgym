...
class SLSolitaireDetailController: YXSBaseTableViewController {
    ...
    @objc func yxs_solitaireClick(sender: YXSButton) {
        ...
        SLSolitaireSelectReasonView(items: detailModel?.optionList ?? [String](), selectedIndex: defaultIndex, title: "选择", inTarget: self.view) { [weak self](view, index) in
            ...
            YXSEducationCensusParentCommitRequest(censusId: weakSelf.censusId ?? 0, childrenId: weakSelf.childrenId ?? 0, option: weakSelf.detailModel?.optionList?[index] ?? "").request({ (json) in
                ...
                weakSelf.navigationController?.yxs_existViewController(existClass: SLSolitaireListController(classId: 0, childId: 0), complete: { (isExist, vc) in
                    vc.yxs_refreshData()
                })
