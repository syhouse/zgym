//
//  SLConversationListController.swift
//  ZGYM
//
...
/// 私聊会话列表
class SLConversationListController: TUIConversationListController, TUIConversationListControllerDelegagte {
    ...
    override func viewDidLoad() {
        ...
        let navRightButton = self.sl_setRightButton(title: "通讯录")
    ...
    // MARK: - Acion
    @objc func contactsClick(sender: SLButton) {
//        let vc = SLContactController(classId: self.sl_user.children?.first?.grade?.id ?? 0)
        let vc = SLContactController()
        self.navigationController?.pushViewController(vc)
    }
    
    @objc func searchClick(sender: SLButton) {
        let vc = SLSearchViewController()
        self.navigationController?.pushViewController(vc)
    }
    ...
    func conversationListController(_ conversationController: TUIConversationListController, didSelectConversation conversation: TUIConversationCell) {
        ...
        if convData.convId == "assistant" {
            let chat = SLAssistantChatViewController(conversation: conv)
            self.navigationController?.pushViewController(chat!)
            
        } else {
            let chat = SLChatViewController(conversation: conv)
            self.navigationController?.pushViewController(chat!)
        }
    }
...
//                let param: Dictionary = SLTCUtil.jsonData2Dictionary(custom.data)
...
//        }
    ...
    // MARK: - LazyLoad
    lazy var btnSearch: SLButton = {
        let btn = SLButton()
        btn.setImage(UIImage(named: "sl_chat_search"), for: .normal)
        btn.setTitle("搜索", for: .normal)
        btn.titleEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 0)
        btn.mixedBackgroundColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9"), night: kNight2C3144)
        btn.setMixedTitleColor(MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#898F9A"), night: kNightFFFFFF), forState: .normal)
        btn.setMixedTitleColor(MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#898F9A"), night: kNightFFFFFF), forState: .highlighted)
