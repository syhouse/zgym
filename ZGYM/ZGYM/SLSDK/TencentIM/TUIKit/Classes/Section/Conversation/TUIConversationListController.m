...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.backgroundColor = SLChatHelper.sharedInstance.isNightTheme ? [UIColor sl_hexToAdecimalColorWithHex:@"#20232F"] : [UIColor whiteColor];
    cell.titleLabel.textColor = SLChatHelper.sharedInstance.isNightTheme ? [UIColor sl_hexToAdecimalColorWithHex:@"#FFFFFF"] : [UIColor blackColor];
    ...
    if ([data.convId isEqualToString:@"assistant"]) {
        ///Custom设置默认头像
        data.avatarImage = [UIImage imageNamed:@"sl_logo"];
        ...
        if (SLChatHelper.sharedInstance.isNightTheme) {
            data.avatarImage = [data.convId containsString:@"TEACHER"] ? [UIImage imageNamed:@"sl_defult_teacher_night"] : [UIImage imageNamed:@"sl_defult_partent_night"];
            
        } else {
            data.avatarImage = [data.convId containsString:@"TEACHER"] ? [UIImage imageNamed:@"sl_defult_teacher"] : [UIImage imageNamed:@"sl_defult_partent"];
        }
