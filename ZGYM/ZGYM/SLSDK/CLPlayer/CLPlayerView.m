...
-(void)cl_playButtonAction:(UIButton *)button{
    ...
    if (self.clickPlayButtonBlock) {
        self.clickPlayButtonBlock(self.maskView.playButton.selected);
    }
    //重新添加工具条定时消失定时器
    [self resetToolBarDisappearTime];
}
...
//MARK:JmoVxia---播放
- (void)playVideo{
    if ([[AVAudioSession sharedInstance].category isEqualToString:AVAudioSessionCategoryPlayback] == NO) {
