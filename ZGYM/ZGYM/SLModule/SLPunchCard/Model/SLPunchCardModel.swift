...
class SLPunchCardModel : NSObject, NSCopying, NSCoding, Mappable{
    func copy(with zone: NSZone? = nil) -> Any {
        let theCopyObj = SLPunchCardModel.init(JSON: self.toJSON())!
        return theCopyObj
    }
    var classId : Int?
    ...
    var endTime : String?
    var clockInDateStatusResponseList : [ClockInDateStatusResponseList]?
    
    var isTop : Int?  //0:否,1:是
    var undo : Bool?//是否可以撤销(对于老师来说 是否是自己发布)
    //是否需要打卡
    ...
    @objc required init(coder aDecoder: NSCoder)
    {
        ...
        clockInDateStatusResponseList = aDecoder.decodeObject(forKey: "clockInDateStatusResponseList") as? [ClockInDateStatusResponseList]
    }
...
class ClockInDateStatusResponseList : NSObject, NSCoding, Mappable{
