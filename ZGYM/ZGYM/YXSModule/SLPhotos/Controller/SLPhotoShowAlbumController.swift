...
class SLPhotoShowAlbumController: YXSBaseViewController,JXCategoryViewDelegate, JXCategoryListContainerViewDelegate {
    ...
    var praisesModels = [Int: SLPhotoAlbumsPraiseModel]()
    ...
    func loadPraiseData(index: Int){
        ...
        YXSEducationAlbumQueryCommentListAndPraiseRequest.init(albumId: model.albumId ?? 0, resourceId: model.id ?? 0).request({ (resultModel: SLPhotoAlbumsPraiseModel) in
    ...
    func loadPraiseOrCancelData(index: Int){
        let model = dataSource[index]
        YXSEducationAlbumPraiseOrCancelRequest.init(albumId: model.albumId ?? 0, resourceId: model.id ?? 0).request({ (resultModel: SLPhotoAlbumsPraiseModel) in
        }) { (msg, code) in
        }
    }
...
//    }
    ...
    lazy var footerView: SLPhotoShowAlubmFooterView = {
       let footerView = SLPhotoShowAlubmFooterView()
        footerView.isHidden = true
...
extension SLPhotoShowAlbumController: YXSRouterEventProtocol{
