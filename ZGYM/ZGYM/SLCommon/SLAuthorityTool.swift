...
class SLAuthorityTool: NSObject{
    // MARK: - 开启媒体资料库/Apple Music 服务
    /// 开启媒体资料库/Apple Music 服务
    @available(iOS 9.3, *)
    static func sl_openMediaPlayerServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        ...
        if authStatus == MPMediaLibraryAuthorizationStatus.notDetermined {
            MPMediaLibrary.requestAuthorization { (status) in
                if (status == MPMediaLibraryAuthorizationStatus.authorized) {
                    ...
                    DispatchQueue.main.async {
                        action(false)
                        if isSet == true {self.sl_OpenURL(.media)}
                    }
                }
            }
            ...
            action(false)
             if isSet == true {sl_OpenURL(.media)}
        }
    }

    // MARK: - 检测是否开启联网
    /// 检测是否开启联网
    static func sl_openEventServiceWithBolck(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        ...
        cellularData.cellularDataRestrictionDidUpdateNotifier = { (state) in
            if state == CTCellularDataRestrictedState.restrictedStateUnknown ||  state == CTCellularDataRestrictedState.notRestricted {
                action(false)
                if isSet == true {self.sl_OpenURL(.network)}
            } else {
                action(true)
            }
        }
        ...
        if state == CTCellularDataRestrictedState.restrictedStateUnknown ||  state == CTCellularDataRestrictedState.notRestricted {
            action(false)
            if isSet == true {sl_OpenURL(.network)}
        } else {
            action(true)
        }
    }
    ...
    // MARK: - 检测是否开启定位
    /// 检测是否开启定位
    static func sl_openLocationServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        var isOpen = false
    ...
    //    }
        ...
        if isOpen == false && isSet == true {sl_OpenURL(.location)}
        action(isOpen)
    }

    // MARK: - 检测是否开启摄像头
    /// 检测是否开启摄像头 (可用)
    static func sl_openCaptureDeviceServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        let authStatus = AVCaptureDevice.authorizationStatus(for: AVMediaType.video)
        if authStatus == AVAuthorizationStatus.notDetermined {
            AVCaptureDevice.requestAccess(for: AVMediaType.video) { (granted) in
                action(granted)
                if granted == false && isSet == true {self.sl_OpenURL(.camera)}
            }
        } else if authStatus == AVAuthorizationStatus.restricted || authStatus == AVAuthorizationStatus.denied {
            action(false)
            if isSet == true {sl_OpenURL(.camera)}
        } else {
            action(true)
        }
    }
    // MARK: - 检测是否开启相册
    /// 检测是否开启相册
    static func sl_openAlbumServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        var isOpen = true
        let authStatus = PHPhotoLibrary.authorizationStatus()
        if authStatus == PHAuthorizationStatus.restricted || authStatus == PHAuthorizationStatus.denied {
            isOpen = false;
            if isSet == true {sl_OpenURL(.photo)}
        }
        action(isOpen)
    }
    ...
    // MARK: - 检测是否开启麦克风
    /// 检测是否开启麦克风
    static func sl_openRecordServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        let permissionStatus = AVAudioSession.sharedInstance().recordPermission
        if permissionStatus == AVAudioSession.RecordPermission.undetermined {
            AVAudioSession.sharedInstance().requestRecordPermission { (granted) in
                action(granted)
                if granted == false && isSet == true {self.sl_OpenURL(.microphone)}
            }
            ...
            action(false)
            if isSet == true {sl_OpenURL(.microphone)}
        } else {
            action(true)
        }
    }
    // MARK: - 跳转系统设置界面
    static func sl_OpenURL(_ type: SLPermissionsType? = nil) {
        let title = "访问受限"
