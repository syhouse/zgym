...
class YXSMineHeaderView: UIView {
    ...
    lazy var btnTag: YXSPaddingLabel = {
        let btn = YXSPaddingLabel()
        if YXSPersonDataModel.sharePerson.personRole == .TEACHER {
        ...
        btn.textInsets = UIEdgeInsets.init(top: 0, left: 10, bottom: 0, right: 10)
        btn.font = UIFont.systemFont(ofSize: 13)
        btn.mixedBackgroundColor = MixedColor(normal: 0xE1EBFE, night: 0x20232F)
        btn.mixedTextColor = MixedColor(normal: 0x5E88F7, night: 0x5E88F7)
        btn.clipsToBounds = true
