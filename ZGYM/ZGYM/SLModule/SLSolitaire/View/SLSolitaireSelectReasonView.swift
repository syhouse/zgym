//
//  SLSolitaireSelectReasonView.swift
//  ZGYM
//
...
import NightNight
/// 我接龙的原因选项
class SLSolitaireSelectReasonView: SLBasePopingView, UITableViewDelegate, UITableViewDataSource {
    
    private var dataSource:[String] = [String]()
    /// 显示TableView的高
    var numberOfHeightByItemCount = 6
    var selectedIndex:Int = -1
    var completionHandler:((_ view:SLSolitaireSelectReasonView, _ selectedIndex:Int)->())?
    
    
    @discardableResult init (items:[String], selectedIndex: Int = -1,title: String = "选择" , inTarget: UIView, completionHandler:((_ view:SLSolitaireSelectReasonView, _ selectedIndex:Int)->())?) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == selectedIndex {
            ...
            cell.btn.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#5E88F7")
    ...
    @objc func doneClick(sender: SLButton) {
        if selectedIndex >= 0 {
            ...
            MBProgressHUD.sl_showMessage(message: "请先选择")
        }
    ...
    lazy var btnLeft : SLButton = {
        ...
        button.sl_addLine(position: .top, color: UIColor.sl_hexToAdecimalColor(hex: "#E3ECFF"), leftMargin: 0, rightMargin: 0, lineHeight: 1)
        button.sl_addLine(position: .right, color: UIColor.sl_hexToAdecimalColor(hex: "#E3ECFF"), leftMargin: 0, rightMargin: 0, lineHeight: 1)
        button.addTarget(self, action: #selector(cancelClick(sender:)), for: .touchUpInside)
        return button
    }()
    
    lazy var btnRight : SLButton = {
        let button = SLButton.init()
