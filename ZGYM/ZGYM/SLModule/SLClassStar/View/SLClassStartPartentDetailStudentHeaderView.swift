//
//  SLClassStartPartentDetailStudentHeaderView.swift
//  ZGYM
//
...
class SLClassStartPartentDetailStudentHeaderView: UIView {
    let stage: StageType
    var childrenModel: SLChildrenModel?
    init(frame: CGRect, stage: StageType) {
        self.stage = stage
        super.init(frame: frame)
        backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#D6E4FF")
    ...
    @objc func lookFirst(){
        SLClassStarApposeView.showAlert(childs: model.mapTop3?.first ?? [SLClassStarChildrenModel](), curruntChild: model.currentChildren, sort:1, stage:  stage)
    }
    @objc func lookSecend(){
        SLClassStarApposeView.showAlert(childs: model.mapTop3?.secend ?? [SLClassStarChildrenModel](), curruntChild: model.currentChildren, sort:2,stage:  stage)
    }
    @objc func lookThrid(){
        SLClassStarApposeView.showAlert(childs: model.mapTop3?.thrid ?? [SLClassStarChildrenModel](), curruntChild: model.currentChildren, sort:3,stage:  stage)
    }
    
    @objc func shareClick(){
        if model != nil{
            UIUtil.sl_shareLink(requestModel: HMRequestShareModel.init(classStarId: childrenModel?.classId ?? 0, childrenId:childrenModel?.id ?? 0, dateType: dateType), shareModel: SLShareModel.init(title: "班级之星", descriptionText: "\(NSUtil.sl_getDateText(dateType: dateType))班级之星已出炉,快来看看\(childrenModel?.realName ?? "")的表现吧~", link: ""))
        }
    }
    
    var model: SLClassStarPartentModel!
    var dateType = DateType.W
    func setHeaderModel(_ model: SLClassStarPartentModel, dateType: DateType,startTime:String?, endTime: String?){
        self.model = model
        ...
        if let startTime = startTime{
            ...
            dayLabel.text = "\(NSUtil.sl_timePeriodText(dateType: dateType))(\(NSUtil.sl_getDateText(dateType: dateType)))"
        }
    ...
    lazy var headerImageView: UIImageView = {
        let imageView = UIImageView.init(image: UIImage.init(named: "sl_classstar_partent_bg"))
        return imageView
    }()
    ...
    lazy var secendRankView: ClassStartPartentRankView = {
        let firstRankView = ClassStartPartentRankView.init(size: CGSize.init(width: 60, height: 60), labelImage: "sl_classstar_partent_secend", labelText: "亚军", borderColor: UIColor.sl_hexToAdecimalColor(hex: "#7567FB"), isFrist: false)
    ...
    lazy var thridRankView: ClassStartPartentRankView = {
        let firstRankView = ClassStartPartentRankView.init(size: CGSize.init(width: 60, height: 60), labelImage: "sl_classstar_partent_thrid", labelText: "季军", borderColor: UIColor.sl_hexToAdecimalColor(hex: "#C2EEFC"), isFrist: false)
    ...
    lazy var shareButton: UIButton = {
        ...
        button.setBackgroundImage(UIImage.init(named: "sl_classstar_share"), for: .normal)
...
class ClassStartPartentRankView: UIView {
    ...
    func setViewModel(_ models: [SLClassStarChildrenModel]?,_ curruntChild: SLClassStarChildrenModel?, index: Int,stage: StageType){
        if let models = models{
            ...
            iconImageView.image = UIImage.init(named: "sl_classstar_student_gray")
            nameLabel.text = "暂无"
            bottomButton.isHidden = true
        }
    }
    
    lazy var bottomButton: SLButton = {
        let bottomButton = SLButton()
        ...
        bottomButton.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#4B73F6")
        return bottomButton
    }()
    ...
    lazy var labelTextLabel: SLLabel = {
        let label = SLLabel()
        label.mixedTextColor = MixedColor(normal: UIColor.white, night: UIColor.white)
        label.font = UIFont.systemFont(ofSize: 14)
        return label
    ...
    lazy var scorelabel: SLLabel = {
        let label = SLLabel()
        label.mixedTextColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#FFE400"), night: UIColor.sl_hexToAdecimalColor(hex: "#FFE400"))
        label.font = UIFont.boldSystemFont(ofSize: 16)
        return label
    }()
}
...
class ClassStartPartentBottomView: UIView {
    ...
    func setViewModel(_ model: SLClassStarPartentModel, dateType: DateType){
        ...
        if let currentChildren = model.currentChildren{
            ...
            if let text = text{
                UIUtil.sl_setLabelAttributed(titleLabel, text: ["恭喜！\(currentChildren.childrenName ?? "")\(NSUtil.sl_getDateText(dateType: dateType))荣获\(text)", "\n您的孩子\(NSUtil.sl_getDateText(dateType: dateType))在班级表现非常榜"], colors: [kTextMainBodyColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 20),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 13)
            }else{
                UIUtil.sl_setLabelAttributed(titleLabel, text: ["孩子名\(NSUtil.sl_getDateText(dateType: dateType))暂未上榜，请再接再厉！", "\n你的孩子\(NSUtil.sl_getDateText(dateType: dateType))表现还有进步空间！"], colors: [kTextMainBodyColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 20),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 13)
            }
            if stage == .KINDERGARTEN{
                UIUtil.sl_setLabelAttributed(scoreLabel, text: ["\(model.currentChildren?.score ?? 0)","朵\n", "\(NSUtil.sl_getDateText(dateType: dateType))小红花"], colors: [kBlueColor, kBlueColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 23),UIFont.boldSystemFont(ofSize: 15),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 12)
                
                UIUtil.sl_setLabelAttributed(avrgerLabel, text: ["\(model.averageScore ?? "")","朵\n", "班级平均分"], colors: [kBlueColor, kBlueColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 23),UIFont.boldSystemFont(ofSize: 15),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 12)
            }else{
                UIUtil.sl_setLabelAttributed(scoreLabel, text: ["\(model.currentChildren?.score ?? 0)\n", "\(NSUtil.sl_getDateText(dateType: dateType))得分"], colors: [kBlueColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 23),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 12)
                UIUtil.sl_setLabelAttributed(avrgerLabel, text: ["\(model.averageScore ?? "")\n", "班级平均分"], colors: [kBlueColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 23),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 12)
            }
            UIUtil.sl_setLabelAttributed(overLabel, text: ["\(model.currentChildren?.topPercent ?? "")%\n", "超越本班同学"], colors: [kBlueColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 23),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 12)
        }else{
            UIUtil.sl_setLabelAttributed(titleLabel, text: ["\(self.sl_user.curruntChild?.realName ?? "")\(NSUtil.sl_getDateText(dateType: dateType))暂未上榜，请再接再厉！", "\n你的孩子\(NSUtil.sl_getDateText(dateType: dateType))表现还有进步空间！"], colors: [kTextMainBodyColor,UIColor.sl_hexToAdecimalColor(hex: "#898F9A")], fonts: [UIFont.boldSystemFont(ofSize: 20),UIFont.boldSystemFont(ofSize: 15)],paragraphLineSpacing: 13)
        }
        if model.showRemindTeacher{
            ...
            if model.currentChildren == nil{
                ...
                bottomView.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F4F7FF")
                bottomView.snp.remakeConstraints { (make) in
                    make.top.equalTo(titleLabel.snp_bottom).offset(15)
                    make.left.right.bottom.equalTo(0)
                    make.height.equalTo(85)
    ...
    lazy var labelImageView: UIImageView = {
        let iconImageView = UIImageView.init(image: UIImage.init(named: "sl_classstar_partent_prize"))
        return iconImageView
    }()
}
