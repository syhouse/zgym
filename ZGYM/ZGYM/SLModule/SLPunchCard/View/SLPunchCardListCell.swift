...
import UIKit
import NightNight
class SLPunchCardListHomeCell: SLPunchCardListCell {
...
class SLPunchCardListCell: SLHomeBaseCell {
    ...
    func sl_setCellModel(_ model: SLPunchCardModel){
    ...
    override func stickClick(){
        if model != nil{
            ...
            if SLPersonDataModel.sharePerson.personRole == .TEACHER && punchCardModel.teacherId == sl_user.id{
                cellBlock?(.cancelStick)
            }
        }
...
extension SLPunchCardListCell{
    func setPunchCardUI(){
        ...
        if isShowTag{
            setTagUI("打卡", backgroundColor: UIColor.sl_hexToAdecimalColor(hex: "#F4E2DF"), textColor: UIColor.sl_hexToAdecimalColor(hex: "#E8534C"))
        }
        ...
        punchTaskLabel.text = "剩余:\(surplusClockInDayCount ?? 0)天  \(self.sl_weakText(periodList: periodList))"
        if SLPersonDataModel.sharePerson.personRole == .PARENT{
            ...
            if showGoToPunch{
                ...
                if hasPunch{
                    ...
                    commonStatusButton.setTitleColor(UIColor.sl_hexToAdecimalColor(hex: "#38B16B"), for: .normal)
                    commonStatusButton.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#DFF3EC")
                    commonStatusButton.setTitle("已打卡", for: .normal)
                }else{
                    punchStatusLabel.text = "今日无需打卡"
                    commonStatusButton.layer.borderWidth = 0
                    commonStatusButton.setTitleColor(UIColor.sl_hexToAdecimalColor(hex: "#E8534C"), for: .normal)
                    commonStatusButton.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F4E2DF")
        ...
        if hasPunchFinish{
            ...
            commonStatusButton.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#C4CDDA")
        ...
        //老师待办区分
        if isAgenda && SLPersonDataModel.sharePerson.personRole == .TEACHER && currentClockInTotalCount == currentClockInPeopleCount {
            punchStatusLabel.text = "今日打卡完成"
        }
        ...
        classLabel.snp.remakeConstraints { (make) in
            ...
            if SLPersonDataModel.sharePerson.personRole == .TEACHER || hasPunch || !hasNeedPunch{
