...
let kYMPhotoCollectionViewDimiss = "SLPhotoCollectionViewDimiss"
let kYMPhotoCollectionViewFinish = "SLPhotoCollectionViewFinish"
let kYMPhotoCollectionViewChoseLocal = "SLPhotoCollectionViewChoseLocal"

class SLPhotoCollectionView: UIView {
    //  最常视频录制时间，单位 秒
    ...
    //  表示当时是否在录像中
    fileprivate let timerTool = SLCountDownTool()
...
//        self.addSubview(focusCursor)
    ...
    @objc fileprivate func cancelClick(){
        self.next?.sl_routerEventWithName(eventName: kYMPhotoCollectionViewDimiss)
    }
    ...
    @objc fileprivate func choseFile(){
        self.next?.sl_routerEventWithName(eventName: kYMPhotoCollectionViewChoseLocal)
    }
                    ...
                    if success {
                        ...
                        if let asset = asset{
                            DispatchQueue.main.async {
                                self.next?.sl_routerEventWithName(eventName: kYMPhotoCollectionViewFinish, info: [kEventKey: asset])
                            }
                        } else {
                            print("视频出错")
    ...
    fileprivate func startCapture(){
        ...
        let filePath: String = NSUtil.sl_cachePath(file: "\(Date()).mp4", directory: kMediaDirectory)
        SLLog(filePath)
    ...
    lazy var resetButton: SLButton = {
        let button = SLButton.init()
        button.setTitleColor(UIColor.white, for: .normal)
        button.setTitle("重录", for: .normal)
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 16)
        button.setBackgroundImage(UIImage.sl_image(with:  UIColor(red: 0, green: 0, blue: 0, alpha: 0.4)), for: .normal)
    ...
    lazy var cancleButton: SLButton = {
        let button = SLButton.init()
        button.setTitleColor(UIColor.white, for: .normal)
    ...
    lazy var selectFileButton: SLCustomImageControl = {
        let selectFileButton = SLCustomImageControl.init(imageSize: CGSize.init(width: 21, height: 21), position: .top, padding: 9.5)
        selectFileButton.font = UIFont.boldSystemFont(ofSize: 15)
        selectFileButton.textColor = UIColor.white
        selectFileButton.title = "选择视频"
        selectFileButton.locailImage = "sl_publish_select_vedio"
        selectFileButton.addTarget(self, action: #selector(choseFile), for: .touchUpInside)
        return selectFileButton
    }()
    
    lazy var cameraSideButton: SLButton = {
        let cameraSideButton = SLButton()
    ...
    lazy var timeLabel: SLLabel = {
        ...
        label.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#222222", alpha: 0.6)
...
extension SLPhotoCollectionView: AVCaptureFileOutputRecordingDelegate{
...
extension SLPhotoCollectionView: CAAnimationDelegate {
    /// 动画开始
...
class CaptureControl: UIControl{
    ...
    override func draw(_ rect: CGRect) {
        ...
        midView.sl_left = (rect.size.width - midView.size.width)/2
        midView.sl_top = (rect.size.height - midView.size.height)/2
