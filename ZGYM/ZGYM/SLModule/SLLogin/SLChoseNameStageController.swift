//
//  SLChoseNameStageController.swift
//  ZGYM
//
//  Created by sl_mac on 2019/12/19.
...
class SLChoseNameStageController: SLBaseViewController {
 
    private var role: PersonRole?
    var stageIndex: Int = 0
    var currentStageName: String = "幼儿园"
    
    var sl_completionHandler: ((_ text:String,_ index:Int, _ vc:SLChoseNameStageController)->())?
    
    init(role: PersonRole?, completionHandler: ((_ text:String,_ index:Int, _ vc:SLChoseNameStageController)->())? = nil) {
        super.init()
        self.sl_completionHandler = completionHandler
...
//        self.btnStage3.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9")
    ...
    @objc func sl_doneClick(sender: SLButton) {
        ...
        if tfInput.text?.count == 0 {
            MBProgressHUD.sl_showMessage(message: "称呼不能为空")
            return
        }
        ...
        if role == .TEACHER {
            let alert = SLConfirmationAlertView.showIn(target: self.view) { [weak self](sender, view) in
                ...
                if sender.titleLabel?.text == "确认" {
                    weakSelf.sl_completionHandler?(weakSelf.tfInput.text ?? "", weakSelf.stageIndex, weakSelf)
                    view.close()
                } else {
                    /// 取消
                }
            ...
            self.sl_completionHandler?(tfInput.text ?? "", stageIndex, self)
        }
    }
...
//        lb.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9")
    ...
    lazy var tfInput: SLQSTextField = {
        let view = SLQSTextField()
...
//        view.textColor = UIColor.sl_hexToAdecimalColor(hex: "#575A60")
        return view
    }()
    ...
    lazy var btnStage1: SLButton = {
        let btn = SLButton()
        btn.tag = 0
        btn.isSelected = true
        btn.setTitle("幼儿园", for: .normal)
        btn.setTitleColor(UIColor.white, for: .selected)
//        btn.setTitleColor(UIColor.sl_hexToAdecimalColor(hex: "#575A60"), for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        btn.clipsToBounds = true
//        btn.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#4E87FF")
        btn.setTitleColor(NightNight.theme == .night ? UIColor.white : UIColor.sl_hexToAdecimalColor(hex: "#575A60"), for: .normal)
        btn.mixedBackgroundColor = MixedColor(normal: 0x4E87FF, night: 0x4E87FF)
        btn.layer.cornerRadius = 2.5
        btn.addTarget(self, action: #selector(sl_stageClick(sender:)), for: .touchUpInside)
        return btn
    }()
    
    lazy var btnStage2: SLButton = {
        let btn = SLButton()
        btn.tag = 1
...
//        btn.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F3F5F9")
        btn.layer.cornerRadius = 2.5
        btn.addTarget(self, action: #selector(sl_stageClick(sender:)), for: .touchUpInside)
        return btn
    }()
    
    lazy var btnStage3: SLButton = {
        let btn = SLButton()
        btn.tag = 2
...
//        btn.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F3F5F9")
    ...
    lazy var btnDone: SLButton = {
        ...
        btn.sl_shadow(frame: CGRect(x: 0, y: 0, width: 318, height: 49), color: kBlueShadowColor, cornerRadius: 24, offset: CGSize(width: 2, height: 2))
...
//        view.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9")
        return view
    }()
    
//    lazy var customNav: HMCustomNav = {
//        let customNav = HMCustomNav()
