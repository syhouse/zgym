//
//  SLAmapLoactionViewController.swift
//  ZGYM
//
...
class SLAmapLoactionViewController: SLBaseTableViewController, MAMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(SLPOITableViewCell.classForCoder(), forCellReuseIdentifier: "SLPOITableViewCell")
//        tableView.register(ZLPOITableViewCell.classForCoder(), forCellReuseIdentifier: "ZLPOITableViewCell")
    ...
    @objc func createNavRightBtnItems() {
        ...
        let btnService = SLButton(frame: CGRect(x: 0, y: 0, width: itemWidth, height: itemWidth))
    ...
    // MARK: - Action
    @objc func coordinateMineClick(sender: SLButton?) {
        self.mapView.shouldUpdatePOI = sender != nil ? true : false
        
        SLAMapManager.share().searchPOIKeyword("学校") { [weak self](response, coordinate, err) in
...
//                }
    ...
    // MARK: -手动输入
    @objc func commitClick(sender: SLButton) {
...
//            if text.count > 0 {
//                MBProgressHUD.sl_showLoading(inView: vc.view)
//                SLAMapManager.share().searchPOIKeyword(text) {(response, coordinate, err) in
//                    MBProgressHUD.sl_hideHUDInView(view: vc.view)
//                    if err == nil {
//                        weakSelf.currentCoordinate = coordinate
...
//
//                    } else {
//                        MBProgressHUD.sl_showMessage(message: err.localizedDescription, inView: vc.view)
//                    }
//                }
//            }
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: SLPOITableViewCell = tableView.dequeueReusableCell(withIdentifier: "SLPOITableViewCell") as! SLPOITableViewCell
    ...
    func mapView(_ mapView: MAMapView!, regionDidChangeAnimated animated: Bool) {
        ...
        if self.mapView.shouldUpdatePOI {
            SLAMapManager.share().searchPOICoordinate(centerCoordinate, keyword: "学校") { [weak self](response, coordinate, err) in
                guard let weakSelf = self else {return}
                weakSelf.currentCoordinate = coordinate
                if response.pois.count == 0 {
                    return
    ...
    // MARK: - LazyLoad
    lazy var mapView: SLMAMapView = {
        let view = SLMAMapView(frame: CGRect.zero)
        view.delegate = self;
    ...
    lazy var btnCoordinateMine: SLButton = {
        let btn = SLButton()
        btn.setImage(UIImage(named: "sl_back_mine_point"), for: .normal)
