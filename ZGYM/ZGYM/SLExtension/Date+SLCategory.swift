...
extension Date {
    /// 时间转换
    static func sl_Time(interval: TimeInterval) -> String {
        let dateTmp: Date = Date(timeIntervalSinceNow: interval / 1000)
        return sl_Time(date: dateTmp)
    }
    
    static func sl_Time(dateStr: String) -> String{
        return sl_Time(date: NSUtil.sl_string2Date(dateStr))
    }
    
    
    /// 全局列表时间 2019.03.14 12:08
    static func sl_Time(date dateTmp: Date) -> String {
...
//        }
    ...
    static func sl_helloTime(interval: TimeInterval) -> String {
