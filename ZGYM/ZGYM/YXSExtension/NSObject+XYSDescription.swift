...
extension NSObject{
    public func yxs_description() -> String{
        do{
            let infoData = try JSONSerialization.data(withJSONObject: self.yxs_covertToDictionary(), options: JSONSerialization.WritingOptions.prettyPrinted)
    ...
    private func yxs_covertToDictionary() -> Dictionary<String, Any?> {
        var count: UInt32 = 0
    ...
    public func yxs_allPropertyNames() ->[String] {
        var count: UInt32 = 0
        let properties = class_copyPropertyList(self.classForCoder, &count)
    ...
    /// 获取对象的所有属性名称跟值
    public func yxs_allPropertys() ->[String : Any?] {
