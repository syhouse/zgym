...
enum SLFriendMessageListType {
    case friends
    case punchCard  //打卡
    case homeWork   //作业
}

class SLCommonMessageListController: YXSBaseTableViewController {
    var loadSucess: (() -> ())?
    ...
    let type:SLFriendMessageListType

    /// 默认朋友圈消息详情
    init(type: SLFriendMessageListType = .friends) {
        self.type = type
        super.init()
    ...
    /// - Parameters:
    ///   - clockId: 打卡id
    convenience init(clockId: Int,isMyPublish: Bool) {
    ...
    func loadData(){
        ...
        case .friends:
            YXSEducationClassCircleMessageListRequest().requestCollection({ (list: [SLFriendsMessageModel]) in
                MBProgressHUD.hide(for: self.view, animated: true)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .punchCard:
            ...
            vc = YXSPunchCardSingleStudentBaseListController.init(clockInId: clockId ?? 0, clockInCommitId: model.clockInCommitId ?? 0, isMyPublish: isMyPublish)
        case .friends:
            let model = dataSource[indexPath.row]
            vc = SLFriendsCircleController.init(classCircleId: model.classCircleId)
        case .homeWork:
