//
//  SLFriendCircleInfoHeaderView.swift
//  ZGYM
//
...
class SLFriendCircleInfoHeaderView: UIView {
    override init(frame: CGRect) {
        super.init(frame: frame)
        mixedBackgroundColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#F2F5F9"), night: kNightForegroundColor)
    ...
    @objc func imageClick(){
        ...
        if model.avatar?.count ?? 0 > 0 {
            imgData.imageURL = URL.init(string: model.avatar ?? "")
        } else  {
            imgData.image = {return self.headerImageSection.rightImage.image}
        }
    ...
    var model:SLFriendCircleUserInfoModel!
    func setHeaderModel(_ model: SLFriendCircleUserInfoModel){
        self.model = model
        var images = [String]()
        headerImageSection.isHidden = true
        if let attachment = model.attachment{
            if attachment.hasSuffix(kHMVedioAppendKey){
                images = [attachment.removingSuffix(kHMVedioAppendKey).sl_getVediUrlImage()]
            }else{
    ...
    @objc func goToFriendDetial(){
        next?.sl_routerEventWithName(eventName: kFriendCircleInfoHeaderDetialEvent)
    }
    ...
    lazy var nameSection: SLCommonSection = {
        let section = SLCommonSection.init(type: SLCommonSectionType.onlyRightTitle)
