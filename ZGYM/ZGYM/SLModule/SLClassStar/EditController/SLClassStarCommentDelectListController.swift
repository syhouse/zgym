...
class SLClassStarCommentDelectListController: SLBaseTableViewController{
    ...
    override func viewDidLoad() {
        ...
        rightButton = sl_setRightButton(title: "批量删除",titleColor: NightNight.theme == .night ? UIColor.white : UIColor.sl_hexToAdecimalColor(hex: "#575A60"))
        ...
        backButton.setMixedTitleColor(MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#575A60"), night: UIColor.white), forState: .normal)
    }
    ...
    func loadDelectData(_ indexPath: IndexPath? ){
        ...
        SLEducationFEvaluationListDeleteRequest.init(evaluationIds: evaluationIds).request({ (json) in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let delectButton = MGSwipeButton.init(title: "", icon: UIImage.init(named: "sl_classstar_edit_delect"), backgroundColor: kBlueColor)
...
extension SLClassStarCommentDelectListController: SLRouterEventProtocol{
...
class ClassStarCommentDelectListCell: MGSwipeTableCell {
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        sl_addLine(position: .bottom, leftMargin: 70, rightMargin: 0)
    ...
    @objc func changeSelectClick(){
        ...
        next?.sl_routerEventWithName(eventName: kClassStarCommentDelectListUpdateUIEvent)
    }

    var model:SLClassStarCommentItemModel!
    func sl_setCellModel(_ model: SLClassStarCommentItemModel){
        self.model = model
        ...
        UIUtil.sl_setLabelAttributed(titleLabel, text: ["\(model.scoreDescribe ?? "")  ", "\(model.evaluationItem ?? "")"], colors: [kBlueColor,UIColor.sl_hexToAdecimalColor(hex: "#575A60")])
