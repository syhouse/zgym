...
class SLClassStartTeacherClassListCell: UITableViewCell {
    ...
    func sl_setCellModel(_ model: SLClassStartClassModel){
        self.model = model
        ...
        if model.score ?? 0 >= 0{
            commentLabel.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#E1EBFE")
            commentLabel.textColor = kBlueColor
        }else{
            commentLabel.backgroundColor = UIColor.sl_hexToAdecimalColor(hex: "#F4E2DF")
            commentLabel.textColor = UIColor.sl_hexToAdecimalColor(hex: "#E8534C")
        }
    }
    // MARK: -getter&setter
    ...
    lazy var commentLabel: SLPaddingLabel = {
