...
let SL_ArchiveFileDirectoryKey = "sl_archiveFile"

public enum SLCacheDirectory:String{
    case HomeWork
    case notice
    case punchCard
    case solitaire
...
class NSUtil {
    ...
    //MARK:File
    static func sl_cachePath() -> String{
    ...
    static func sl_archiveFile(file: String) -> String{
        return sl_cachePath(file: file, directory: SL_ArchiveFileDirectoryKey)
    }
    
    /// audio存储地址
    /// - Parameter directory: CacheDirectory
    /// - Parameter id: 提交业务id
    static func sl_recordCachePath(_ directory :SLCacheDirectory, _ id: String) -> String{
        let dir = sl_cachePath().appendingPathComponent(kMediaDirectory)
        ...
        let str = SLPersonDataModel.sharePerson.personRole == .PARENT ? "Parent": "Teacher"
    ...
    static func sl_cachePath(file: String,directory: String = "") -> String{
        let dir = sl_cachePath().appendingPathComponent(directory)
        if !isDirectoryExist(path: dir){
            do{
                try FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true, attributes: nil)
            }catch{
    ...
    static func sl_removeCachePathFile(file: String){
        let dir = sl_cachePath()
...
// MARK: - 阿里云上传资源路径
    static func getPath(sourceType: SourceNameType,fileName: String,fileType: String) -> String{
        return "\(sourceType.rawValue)/" + "iOS/" + "\(SLPersonDataModel.sharePerson.userModel.id ?? 0)/" + "\(SLPersonDataModel.sharePerson.userModel?.type ?? "")/"  + fileName + "." + fileType
    }
    /// 编译版本(6)
    ...
    ///   - string: 字符串
    ///   - dateFormat: 日期格式
    static func sl_string2Date(_ string:String, dateFormat:String = "yyyy-MM-dd HH:mm:ss") -> Date {
    ...
    ///   - date1: date1
    ///   - date2: date2
    static func sl_isSameDay(_ date1: Date, date2: Date) -> Bool {
    ...
    ///   - width: h宽度
    ///   - height: 高度
    static func sl_imageThumbnail( width: Int, height: Int) -> String {
    ...
    static func sl_timePeriodText(dateType: DateType) -> String{
    ...
    static func sl_timeDayText(dateType: DateType) -> (x: String, y: String){
...
//            + "-"
    ...
    /// 班级之星日期字符串
    /// - Parameter dateType: 日期格式
    static func sl_getDateText(dateType: DateType) -> String{
        switch dateType {
            case .D:
            return "今日"
        case .W:
    ...
    /// 链接编码
    /// - Parameter url: 中文链接
    static func sl_urlAllowedCharacters(url: String) -> String{
