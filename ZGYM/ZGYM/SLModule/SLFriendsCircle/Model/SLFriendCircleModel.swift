...
class SLFriendCircleModel : NSObject, NSCoding, Mappable{
    
    var attachment : String?{
        didSet{
            if let attachment = attachment{
                var imgs = [SLFriendsMediaModel]()
                if attachment.hasSuffix(kHMVedioAppendKey){
                    let img = SLFriendsMediaModel.init(url: attachment.removingSuffix(kHMVedioAppendKey), type: .serviceVedio)
                    imgs.append(img)
                    isVideoSource = true
                }else{
                    ...
                    for pic in pics{
                        if pic.hasPrefix("friends_guide"){
                            let img = SLFriendsMediaModel.init(url: pic, type: .locailImg)
                            imgs.append(img)
                        }else{
                            let img = SLFriendsMediaModel.init(url: pic, type: .serviceImg)
                            imgs.append(img)
                        }
    ...
    var praises :[SLFriendsPraiseModel]?
    var comments: [SLFriendsCommentModel]?
    ...
    var isMyPublish:Bool{
        get{
            return userIdPublisher == sl_user.id && typePublisher == sl_user.type
        }
    }
    ...
    /// 朋友圈
    var frameModel:SLFriendsCircleFrameModel!
    /// 朋友圈高度
        ...
        get{
            if frameModel != nil{
                ...
                if let prises = self.praises, prises.count > 0{
                    ...
                    let favsHeight = UIUtil.sl_getTextHeigh(textStr: favsStr, attributes: attributes, width: SCREEN_WIDTH - helper.contentLeftMargin - 31 - 15 - 15) + 2
    ...
    func confingHeight(){
        let helper = SLFriendsConfigHelper.helper
        frameModel = SLFriendsCircleFrameModel()
        let paragraphStye = NSMutableParagraphStyle()
        //调整行间距
        ...
        frameModel.contentIsShowAllHeight = UIUtil.sl_getTextHeigh(textStr: content ?? "", attributes: attributes, width: helper.contentWidth) + 1
        var text: String = content ?? ""
        frameModel.contentHeight = UIUtil.sl_getTextHeigh(textStr: text.removeSpace() , attributes: attributes,width: helper.contentWidth, numberOfLines: 3) + 1
    ...
    @objc required init(coder aDecoder: NSCoder)
    {
        ...
        comments = aDecoder.decodeObject(forKey: "comment") as? [SLFriendsCommentModel]
        ...
        praises = aDecoder.decodeObject(forKey: "praise") as? [SLFriendsPraiseModel]
        ...
        imgs = aDecoder.decodeObject(forKey: "imgs") as? [SLFriendsMediaModel]
        isVideoSource = aDecoder.decodeBool(forKey: "isVideoSource")
        frameModel = aDecoder.decodeObject(forKey: "frameModel") as? SLFriendsCircleFrameModel
    }
