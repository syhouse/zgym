...
/// 带关闭/确认按钮的输入框
class SLInputAlertView: UIView {

    var complete: ((_ text: String, _ clickButton:SLButton)->())?
    private var maxLength:Int = 11
    
    static func showIn(target: UIView, maxLength:Int = 11, complete:((_ text: String, _ clickButton:SLButton) ->())?) -> SLInputAlertView{
        let view = SLInputAlertView()
...
//        }
    ...
    lazy var tfInput: SLQSTextField = {
        ...
        tf.mixedBackgroundColor = MixedColor(normal: UIColor.sl_hexToAdecimalColor(hex: "#F3F5F9"), night: kNightForegroundColor)
    ...
    lazy var btnDone: SLButton = {
        ...
        btn.sl_addLine(position: LinePosition.top, color: UIColor.sl_hexToAdecimalColor(hex: "#E6EAF3"), leftMargin: 0, rightMargin: 0, lineHeight: 1)
        return btn
    }()
