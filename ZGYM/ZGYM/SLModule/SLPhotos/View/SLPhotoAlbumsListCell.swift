...
class SLPhotoAlbumsListCell: UICollectionViewCell {
    ...
    func setCellModel(_ model: SLPhotoAlbumsModel){
        ...
        if model.isSystemCreateItem{
            imageView.image = UIImage.init(named: "sl_photo_new_ablum")
            createLabel.isHidden = false
        }else{
            
            imageView.sd_setImage(with: URL.init(string: (model.coverUrl ?? "").sl_getImageThumbnail()), placeholderImage: UIImage.init(named: "sl_photo_nocover"))
    ...
    lazy var titleLabel: UILabel = {
       ...
       lazy var countLabel: UILabel = {
        ...
        view.textColor = UIColor.sl_hexToAdecimalColor(hex: "#898F9A")
        return view
    }()
    ...
    lazy var createLabel: UILabel = {
        ...
        view.textColor = UIColor.sl_hexToAdecimalColor(hex: "#B2B9C6")
        view.text = "新建相册"
        return view
    }()
}
