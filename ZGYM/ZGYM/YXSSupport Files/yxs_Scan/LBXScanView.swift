...
open class LBXScanView: UIView {
    ...
    // 网格扫码动画封装
    var scanNetAnimation: YXSLBXScanNetAnimation?
    ...
    */
    public init(frame: CGRect, vstyle: LBXScanViewStyle) {
        ...
        case LBXScanViewAnimationStyle.NetGrid:
            scanNetAnimation = YXSLBXScanNetAnimation.instance()
    ...
    func drawScanRect() {
        ...
        // 扫码区域Y轴最小坐标
        let SLinRetangle = frame.size.height / 2.0 - sizeRetangle.height / 2.0 - viewStyle.centerUpOffset
        let SLaxRetangle = SLinRetangle + sizeRetangle.height
        ...
        // 填充矩形
        // 扫码区域上面填充
        var rect = CGRect(x: 0, y: 0, width: frame.size.width, height: SLinRetangle)
        context.fill(rect)

        // 扫码区域左边填充
        rect = CGRect(x: 0, y: SLinRetangle, width: XRetangleLeft, height: sizeRetangle.height)
        context.fill(rect)

        // 扫码区域右边填充
        rect = CGRect(x: XRetangleRight, y: SLinRetangle, width: XRetangleLeft, height: sizeRetangle.height)
        context.fill(rect)

        // 扫码区域下面填充
        rect = CGRect(x: 0, y: SLaxRetangle, width: frame.size.width, height: frame.size.height - SLaxRetangle)
        context.fill(rect)
        // 执行绘画
        context.strokePath()
        ...
        if viewStyle.isNeedShowRetangle {
            ...
            context.addRect(CGRect(x: XRetangleLeft, y: SLinRetangle, width: sizeRetangle.width, height: sizeRetangle.height))

            // CGContextMoveToPoint(context, XRetangleLeft, SLinRetangle);
            // CGContextAddLineToPoint(context, XRetangleLeft+sizeRetangle.width, SLinRetangle);

            context.strokePath()
        }

        scanRetangleRect = CGRect(x: XRetangleLeft, y: SLinRetangle, width: sizeRetangle.width, height: sizeRetangle.height)
        ...
        let topY = SLinRetangle - diffAngle
        let rightX = XRetangleRight + diffAngle
        let bottomY = SLaxRetangle + diffAngle
    ...
    static func getScanRectWithPreView(preView: UIView, style: LBXScanViewStyle) -> CGRect {
        ...
        // 扫码区域Y轴最小坐标
        let SLinRetangle = preView.frame.size.height / 2.0 - sizeRetangle.height / 2.0 - style.centerUpOffset
        // 扫码区域坐标
        let cropRect = CGRect(x: XRetangleLeft, y: SLinRetangle, width: sizeRetangle.width, height: sizeRetangle.height)
    ...
    func deviceStartReadying(readyStr: String) {
        ...
        if activityView == nil {
            ...
            activityView?.center = CGPoint(x: XRetangleLeft + sizeRetangle.width / 2 - 50, y: SLinRetangle + sizeRetangle.height / 2)
