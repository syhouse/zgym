...
/// 带标题副标题/确认/取消 的弹窗
class SLConfirmationAlertView: UIView {
    
    var complete: ((_ sender: UIButton,_ view: SLConfirmationAlertView)->())?
    var hasCancel: Bool!
    static func showIn(target: UIView, hasCancel:Bool = true, complete:((_ sender: UIButton, _ view: SLConfirmationAlertView)->())?) -> SLConfirmationAlertView{
        let view = SLConfirmationAlertView(hasCancel: hasCancel)
    ...
    func layout() {
        ...
        lazy var btnCancel: UIButton = {
            ...
            btn.sl_addLine(position: LinePosition.top, color: UIColor.sl_hexToAdecimalColor(hex: "#E6EAF3"), leftMargin: 0, rightMargin: 0, lineHeight: 0.5)
            btn.sl_addLine(position: LinePosition.right, color: UIColor.sl_hexToAdecimalColor(hex: "#E6EAF3"), leftMargin: 0, rightMargin: 0, lineHeight: 0.5)
            return btn
