//
//  SLNoticeReadListViewController.swift
//  ZGYM
//
//  Created by sl_mac on 2019/12/24.
...
class SLNoticeReadListViewController: SLBaseTableViewController, JXCategoryListContentViewDelegate {
    ...
    @objc func reminderRequest() {
        ...
        SLEducationTeacherOneTouchReminderRequest(childrenIdList: childrenIdList, classId: SLHomeListModel?.classId ?? 0, opFlag: 0, serviceId: SLHomeListModel?.serviceId ?? 0, serviceType: 0, serviceCreateTime: SLHomeListModel?.createTime ?? "").request({ (json) in
